...
!!$  real(kind=DBL_KIND),parameter :: Rhocr = 1.D-13/1.D-16  ! critical density betweeen isothermal and polytrope
  real(kind=DBL_KIND),parameter :: Rhocr = 1.D-13/ModelParam_rho  ! critical density betweeen isothermal and polytrope
!!$  real(kind=DBL_KIND),parameter :: Rhocr = 0.1d0
...
!!$#define FLMT(x,y) (x)*0.d0+(y)*0.d0
  subroutine flux(u, f1d, ncrd)
    ...
    real(kind=DBL_KIND) :: &
         ...
         ea7, eb1, eb2, eb3, eb4, eb5, eb6, eb7,  el1, el2, el3, &
         ...
         rhor,vxr,vyr,vzr,bxr,byr,bzr,psir, &
         rholl,vxll,vyll,vzll,bxll,byll,bzll,psill, &
         rhorr,vxrr,vyrr,vzrr,bxrr,byrr,bzrr,psirr, &
         csbar, cs2, &
