...
!!$  real(kind=DBL_KIND),save :: Cs
  real(kind=DBL_KIND),save :: Rhocr=0.d0 ! dummy
  real(kind=DBL_KIND),save :: Gamma=1.d0 ! dummy
  real(kind=DBL_KIND),save :: Kappa=1.d0 ! dummy
contains
  !-----------------------------------------------------------------------
...
#define DTI_(V_,G_,H_) sqrt((V_)**2+2.d0*(G_)*(H_))/(H_)
             dt = min(dt, &
                  (CFL) / ( &
                  DTI_( abs(vx(i,j,k))+Cs, abs(gx(i,j,k)), h(MX) ) + &
                  DTI_( abs(vy(i,j,k))+Cs, abs(gy(i,j,k)), h(MY) ) + &
                  DTI_( abs(vz(i,j,k))+Cs, abs(gz(i,j,k)), h(MZ) ) ))
#else   !WITH_SELFGRAVITY
             dt = min(dt, &
                  (CFL) / ( &
                  (abs(vx(i,j,k))+Cs)/h(MX) + &
                  (abs(vy(i,j,k))+Cs)/h(MY) + &
                  (abs(vz(i,j,k))+Cs)/h(MZ) ))
#endif  !WITH_SELFGRAVITY
          enddo
       enddo
    enddo
...
subroutine eos_init
!!$  use modelParameter, only : MP_rhoEnvelope, MP_prEnvelope
!!$  use eos
!!$  Cs = sqrt(MP_prEnvelope/MP_rhoEnvelope)
