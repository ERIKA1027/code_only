...
#define FH 11
  ...
  integer,parameter :: NGI=(NGI_BASE)*(NI)*2**(Lmax), NGJ=(NGJ_BASE)*(NJ)*2**(Lmax) &
    , NGK=(NGK_BASE)*(NK)*2**(Lmax)
...
!  complex(KIND=CMPLX_KIND),dimension(:,:,:),allocatable :: ax, ay, az, vx, vy, vz, work
  ...
  real(KIND=DBL_KIND) :: sigma_v_local, sigma_v, MP_Boxsize, MP_Lmax0, MR_Rcloud
  !
  ...
  !get Boxsize --------------------------------------------
  !if( readenv('Lmax0', MP_Lmax0) ) then
  !      print *, 'Lmax0  =', MP_Lmax0, '[au]'             ! (half) boxsize in au
  !else
  !      print *, '***********', 'error in readind Boxsize'
  !      stop
  !endif
  !if(Lmax < MP_Lmax0)then
  !      print *, 'Lmax < MP_Lmax0'
  !      stop
  !endif
  ...
  do k = 0, NGK/2              ! global K
     kz(k) = dble(k)/ZMAX
  end do
  do k = NGK/2+1, NGK-1        ! global K
     kz(k) = (dble(k)-NGK)/ZMAX
  end do
  !
