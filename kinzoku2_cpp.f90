...
contains
  subroutine CIIcool(xnH,T_K,Trad,xNc_CII,y_m,y_a,y_e,esc_10,tau_c,xLd_CII)
    implicit none
    real(kind=8) :: xnH,T_K,Trad,xNc_CII,y_m,y_a,y_e,esc_10,tau_c,xLd_CII
    real(kind=8), parameter :: xk_B = 1.38d-16
    real(kind=8), parameter :: h_P = 6.63d-27
    real(kind=8), parameter :: xm_p = 1.67d-24
    real(kind=8), parameter :: pi = 3.14159265358979d0
    type(line2) :: l2
    real(kind=8) :: DT_10, DE_10, xnu_10, gamma_e, gamma_H, gamma_H2
    real(kind=8) :: xm_C, v_th, f_0, f_1,S_10
    l2%tau_cnt=tau_c
    l2%g_0=2.d0
    l2%g_1=4.d0
    DT_10=92.d0
    ...
    xLd_CII=DE_10*l2%A_10*f_1*esc_10*(1.d0-l2%Q_10/S_10) 
    return
  end subroutine CIIcool
  subroutine CIIpop(f_0,f_1,esc_10, l2)
    implicit none
    type(line2) :: l2
    real(kind=8) :: f_0, f_1, esc_10
    real(kind=8), parameter :: err_eps=1.d-4
    real(kind=8) :: esc(1),error(1),desc(1),esc_f(1),error_f(1),A(1,1)
    real(kind=8) :: err_max, fact
  ...
  subroutine twolevel(f_0,f_1,esc,error,l2)
    implicit none
    type(line2) :: l2
    real(kind=8) :: f_0, f_1, esc_10, R_01, R_10, tau_10
    real(kind=8) :: esc(1),error(1)
    esc_10=esc(1)
  ...
  subroutine OIcool(xnH,T_K,Trad,xNc_OI,y_m,y_a,y_e,esc,tau_c,xLd_OI)
    implicit none
    real(kind=8) :: xnH,T_K,Trad,xNc_OI,y_m,y_a,y_e,esc(1:3),tau_c,xLd_OI
    real(kind=8), parameter :: xk_B = 1.38d-16
    real(kind=8), parameter :: h_P = 6.63d-27
    real(kind=8), parameter :: xm_p = 1.67d-24
    type(line3) :: l3
    real(kind=8) :: DT_10, DT_20, DT_21, DE_10, DE_20, DE_21 &
    ...
    xLd_OI=(DE_10*l3%A_10*f_1*x_10+DE_20*l3%A_20*f_2*x_20+DE_21*l3%A_21*f_2*x_21) 
    end subroutine OIcool
  subroutine H2Ocool(xnH,T_K,y_H2,xNc_H2O,tau_cnt,xLd_H2O)
    implicit none
    real(kind=8), parameter :: xk_B = 1.38d-16
    real(kind=8), parameter :: xm_p = 1.67d-24
    real(kind=8) :: xnH,T_K,y_H2,xNc_H2O,tau_cnt,xLd_H2O
    real(kind=8) :: xn_c, xlT, cs, xlN, aL0, aLLTE, xlnh, alpha &
        , xL0inv, xLLTEinv, xn_h, xLinv, xL, xlNo, xLo, xlNp, xLp, aL0p
    real(kind=8) :: xlTa(1:6)=(/2.000d0, 2.301d0, 2.602d0, 3.000d0, 3.301d0, 3.602d0/)
    real(kind=8) :: xlNa(1:10)=(/10.0d0, 11.0d0, 12.0d0, 13.0d0, 14.0d0, &
          15.0d0, 16.0d0, 17.0d0, 18.0d0, 19.0d0/)
    real(kind=8) :: aL0a(1:6)=(/24.35d0, 23.87d0, 23.42d0, 22.88d0, 22.50d0, 22.14d0/)
    real(kind=8) :: aLLTEa(1:6,1:10)=reshape((/14.59d0 &
     ...
     real(kind=8) :: xlnha(1:6,1:10) = reshape((/9.00d0&
     ...
     real(kind=8) :: alphaa(1:6,1:10) = reshape((/0.43d0 &
      ...
      real(kind=8) :: xlTb(1:6)=(/1.000d0, 1.301d0, 1.477d0, &
          1.699d0, 1.903d0, 2.000d0/)
      real(kind=8) :: aL0bo(1:6)=(/26.81d0, 25.88d0, 25.43d0,&
          24.96d0, 24.58d0, 24.41d0/)
      real(kind=8) :: aLLTEbo(1:6,1:10)=reshape((/17.94d0 &
      ...
      real(kind=8) :: xlnhbo(1:6,1:10) = reshape((/8.81d0 &
      ...
      real(kind=8) :: alphabo(1:6,1:10) =reshape((/0.71d0 &
      ...
      real(kind=8) :: aL0bp(1:6)=(/27.01d0, 25.73d0, 25.24d0, &
           24.75d0, 24.38d0, 24.22d0/)
      real(kind=8) :: aLLTEbp(1:6,1:10)=reshape ((/17.72d0&
      ...
      real(kind=8) :: xlnhbp(1:6,1:10) =reshape((/9.30d0 &
      ...
      real(kind=8) :: alphabp(1:6,1:10) =reshape ((/0.49d0 &
  ...
  subroutine OHcool(xnH,T_K,y_H2,xNc_OH,tau_cnt,xLd_OH)
    ...
    real(kind=8), parameter :: xk_B = 1.38d-16
    real(kind=8), parameter :: xm_p = 1.67d-24
    real(kind=8) :: xnH, T_K, y_H2, xNc_OH, tau_cnt, xLd_OH
    real(kind=8) :: xn_c, xlT, cd, xlN, aL0, aLLTE, xlnh, alpha
    real(kind=8) :: xL0inv, xLLTEinv, xn_h, xLinv, xL
    real(kind=8) :: xlTa(1:6)=(/1.477d0, 1.699d0, 1.903d0, 2.000d0, 2.477d0, 2.778d0/)
    real(kind=8) :: xlNa(1:9)=(/10.0d0, 11.d0, 12.0d0, 13.d0, 14.d0, 15.d0 &
      , 16.d0, 17.d0, 18.d0/)
    real(kind=8) :: aL0a(1:6)= &
        (/0.2531d2, 0.2453d2, 0.2402d2, 0.2382d2, 0.2316d2, 0.2290d2/)
    real(kind=8) :: aLLTEa(1:6,1:9) = reshape((/ 0.1620d2 &
    ...
    real(kind=8) :: xlnha(1:6,1:9) = reshape((/ 0.905d1, &
    ...
    real(kind=8) :: alphaa(1:6,1:9) = reshape((/ 0.353d0 &
...
14 continue
  ...
  subroutine HDcool(xnH,T_K, Trad,y_H,y_H2,xNc_HD,tau_cnt,xLd_HD)
    implicit none
    real(kind=8), parameter :: xk_B = 1.38066d-16
    real(kind=8), parameter :: h_Pl = 6.62618d-27
    real(kind=8), parameter :: pi = 3.14159265358979d0
    real(kind=8), parameter :: xm_p = 1.67d-24
    real(kind=8) :: xnH,T_K, Trad,y_H,y_H2,xNc_HD,tau_cnt
    real(kind=8) :: g0, g1, g2, g3
    real(kind=8) :: a10d, a21d, a32d, a20d, a31d
    real(kind=8) :: xnu10d, xnu21d, xnu32d, xnu20d, xnu31d
    real(kind=8) :: DT10d, DT21d, DT32d, DT20d, DT31d
    real(kind=8) :: c10d, c21d, c32d, c20d, c31d, c01d, c12d, c23d, c02d, c13d
    real(kind=8) :: q10d, q21d, q32d, q20d, q31d
    real(kind=8) :: w10d, w21d, w32d, w20d, w31d, w30d, w01d, w12d, w23d, w02d, w13d, w03d
    real(kind=8) :: w0d, w1d, w2d, an0d, an1d, an2d, an3d, antotd
    real(kind=8) :: f0d, f1d, f2d, f3d, xNc0d, xNc1d, xNc2d, xNc3d, v_D, tau10d, tau21d, tau32d, tau20d, tau31d
    real(kind=8) :: esc10d, esc21d, esc32d, esc20d, esc31d, s10d, x10d, s21d, s20d, x21d &
      , x20d, s32d, s31d, x32d, x31d
    real(kind=8) :: xLd10d, xLd21d, xLd32d, xLd20d, xLd31d, xLd_HD
    g0=1.d0
    g1=3.d0
    g2=5.d0
    g3=7.d0
    ...
    xLd10d = x10d*f1d*a10d*h_Pl*xnu10d 
    xLd21d = x21d*f2d*a21d*h_Pl*xnu21d 
    xLd32d = x32d*f3d*a32d*h_Pl*xnu32d 
    xLd20d = x20d*f2d*a20d*h_Pl*xnu20d 
    xLd31d = x31d*f3d*a31d*h_Pl*xnu31d 
  ...
  subroutine CIcool(xnH,T_K, Trad,xNc_CI,y_m,y_a,y_e,esc,tau_c,xLd_CI)
    implicit none
    real(kind=8) :: xnH,T_K, Trad,xNc_CI,y_m,y_a,y_e,tau_c,xLd_CI
    real(kind=8) :: esc(1:3)
    real(kind=8), parameter :: xk_B = 1.38d-16
    real(kind=8), parameter :: h_P = 6.63d-27
    real(kind=8), parameter :: xm_p = 1.67d-24
    real(kind=8) :: DT_10, DT_20, DT_21, DE_10, DE_20, DE_21
    real(kind=8) :: gamma10_e, gamma20_e, gamma21_e, gamma10_H, gamma20_H, gamma21_H
    real(kind=8) :: gamma10_H2, gamma20_H2, gamma21_H2
    real(kind=8) :: xm_C, f_0, f_1, f_2, S_10, S_20, S_21, x_10, x_20, x_21
    type(line3) :: l3
    if(T_K < 1.d0) then
       xLd_CI=0.d0
       return
    ...
    x_20=esc(2)*(1.d0-l3%Q_20/S_20)
    x_21=esc(3)*(1.d0-l3%Q_21/S_21)
    xLd_CI=(DE_10*l3%A_10*f_1*x_10+DE_20*l3%A_20*f_2*x_20+DE_21*l3%A_21*f_2*x_21) 
    return
  end subroutine CIcool
  subroutine pop3lev(f_0,f_1,f_2,esc,l3)
    implicit none
    real(kind=8), parameter :: err_eps = 1.d-5
    real(kind=8), dimension(3) :: esc,error,desc,esc_f,error_f
    real(kind=8), dimension(3,3) :: A
    integer :: itr, i, j, jj
    real(kind=8) :: f_0,f_1,f_2
    real(kind=8) :: err_max, delta_esc,fact
  ...
  subroutine thrlev(f_0,f_1,f_2,esc,error,l3)
    ...
    type(line3) :: l3
    real(kind=8) :: f_0,f_1,f_2
    real(kind=8),dimension(3) :: esc, error
    real(kind=8), parameter :: pi=3.14159265358979d0
    real(kind=8) :: esc_10, esc_20, esc_21
    real(kind=8) :: R_10, R_20, R_21, R_01, R_02, R_12
    real(kind=8) :: xNc_0, xNc_1, xNc_2, tau_10, tau_20, tau_21
    real(kind=8) :: err_10, err_20, err_21
    esc_10=esc(1)
    esc_20=esc(2)
    esc_21=esc(3)
  ...
  function Q_bg(T_nu, Trad)
    real(kind=8) :: Q_bg
    real(kind=8),intent(IN) :: T_nu, Trad
    real(kind=8) :: x, Q_bg_CMB
    x = T_nu/Trad
    if(x.gt.1.d2) then
       Q_bg_CMB = 0.d0
    else
