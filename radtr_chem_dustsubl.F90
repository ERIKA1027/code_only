  ...
  subroutine ch_radtrchem(dt_code, dt_hyd, rsm)
    ...
    do level = Lmin, Lmax
      ...
      xch%rcopd = 0.d0
              ...
              !----------EO_added----------!
              if(sqrt(x(i)**2.0+y(j)**2.0+z(k)**2.0)>MP_dustsubl) then
                sig_euv = xch%fd * rsm%sig_EUV  ! 融解 & Metallicity 込み
                sig_fuv = xch%fd * rsm%sig_FUV
              else
              sig_euv = 0.d0  ! 融解 & Metallicity 込み
              sig_fuv = 0.d0 
              endif
              !----------EO_added----------!
      ...
      #if defined(M1CLOSER_FUV_TRANSFER) && defined(M1CLOSER_SEPARATE_FUV_TRANS)
              xch%rdph=(rsm%erg_EUV*rsm%sig_EUV*Nph_euv + rsm%erg_FUV*rsm%sig_FUV*Nph_fuv_dust)*MP_Ctil*Unit_invmu ! ダスト別に計算
              !----------EO_added----------!
             ! if(sqrt(x(i)**2.0+y(j)**2.0+z(k)**2.0)>MP_dustsubl) then
             ...
             ! endif
              !----------EO_added----------!
      #elif defined(M1CLOSER_FUV_TRANSFER) 
              xch%rdph=(rsm%erg_EUV*rsm%sig_EUV*Nph_euv + rsm%erg_FUV*rsm%sig_FUV*Nph_fuv)*MP_Ctil*Unit_invmu ! FUV一色
              !----------EO_added----------!
             !  if(sqrt(x(i)**2.0+y(j)**2.0+z(k)**2.0)>MP_dustsubl) then
      ...
      #else
              xch%rdph=rsm%erg_EUV*rsm%sig_EUV*Nph_euv*MP_Ctil*Unit_invmu 
             ...
             !----------EO_added----------!
              ...
              !----------EO_added----------!
        !      call dtemp_radtr(xch%nH, xch%Tg, xch%Td, Erad, chi_d, dph)
...
!                cf_EUV = MP_PHON*rsm%erg_EUV*(rsm%alpha_EUV*(xch%ychem(X_HI)+xch%ychem(X_H2))+sig_euv)*Unit_invmu/cgs_c/(Unit_v*Unit_l**2)*(dt_code/dt_hyd) ! erg cm^2 g^-1 cm^-1 s => cm^3 s^-1
      if(sqrt(x(i)**2.d0+y(j)**2.d0+z(k)**2.d0)>MP_dustsubl) then
        ...
        cf_EUV = MP_PHON*rsm%erg_EUV*(rsm%alpha_EUV*(xch%ychem(X_HI)+xch%ychem(X_H2)))*Unit_invmu/cgs_c/(Unit_v*Unit_l**2)*(dt_code/dt_hyd) ! erg cm^2 g^-1 cm^-1 s => cm^3 s^-1 
      endif
      #endif
