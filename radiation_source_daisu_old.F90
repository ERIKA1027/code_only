...
subroutine radiation_source  
  ...
  !----------EO_added----------!
   ...
   real(kind=DBL_KIND) :: eta_edd, lum_edd, lum0
  !----------EO_added----------!
...
!     lum = lum * cgs_lsun                             !luminosity in [erg s^-1]
     ...
     !-----------EO_added-----------!
!     lum = eta * mdot * (Unit_m / Unit_t) * cgs_c**2.d0

!      eta_edd = 1.0d-2
!      lum_edd = 4.d0 * pi * cgs_c * cgs_gc * mass * Unit_m  * cgs_mp / sigma_tom  
!      lum_edd = 4.d0 * pi * cgs_c * cgs_gc * 4.d2 * cgs_msun  * cgs_mp / sigma_tom  
!      lum     = eta_edd * lum_edd    

      p      = 1.d0 / 3.d0
      nu_min = 1.12d1 * cgs_ev / cgs_hpk
      nu_max = 1.d-1 * 1.d3 * cgs_ev / cgs_hpk
      nu0    = 1.d16
      lum0   = 1.d36
      lum    = lum0 * (nu_max**(p + 1.d0) - nu_min**(p + 1.d0)) / ((p + 1.d0)  * nu0**p)
     !-----------EO_added-----------!
...
!     lum0 = lum
     ...
     nu_euv_max = 1.d-1 * 1.d3 * cgs_ev / cgs_hpk
     S_euv      = lum0 *(nu_euv_max**p - nu_euv_min**p) / (cgs_hpk * p * nu0**p)
     rda%xeuv   = S_euv / lum
!     rda%xeuv   = (S_euv / lum) * Unit_t 


     !for xfuv!
     nu_fuv_min = 1.12d1 * cgs_ev / cgs_hpk
     nu_fuv_max = 1.36d1 * cgs_ev / cgs_hpk
     S_fuv      = lum0 *(nu_fuv_max**p - nu_fuv_min**p) / (cgs_hpk * p * nu0**p)
     rda%xfuv   = S_fuv / lum
!     rda%xfuv   = (S_fuv / lum) * Unit_t

     !for alpha_euv!
!     sigma_mean    = 6.3d0 * 1.d-18 * lum * nu_euv_min**3.d0 *(nu_euv_max**(p-3.d0) - nu_euv_min**(p - 3.d0)) / (cgs_hpk * nu0**p * (p - 3.d0))
     sigma_mean    = 6.3d0 * 1.d-18 * S_euv
     rda%alpha_euv = sigma_mean / S_euv   
!     rda%alpha_euv = (sigma_mean / S_euv) / (Unit_l**2.d0)     
...
!     rda%heat_euv = ((heat_cal1 - heat_cal2) / S_euv) / Unit_erg
     ...
     !for other!
!     lum        = lum / (Unit_erg / Unit_t)

     rda%hhm    = 0.d0
     #ifdef METAL
     rda%sig_euv = 1.d-17
!     rda%sig_euv = 1.d-17 * (Unit_l**2.d0)

     rda%sig_fuv = 1.d-21
!     rda%sig_fuv = 1.d-21 * (Unit_l**2.d0)
...
!  #endif
     ...
     rs_info%sig_euv(n_rs)    = 1.d-17     ! dust cross section for euv photons [cm^-2]
