...
subroutine radiation_source  
  ...
  !----------EO_added----------!
 ! real :: mass
  real(kind=DBL_KIND),parameter :: eta = 1.d-1 !efficiency of the release of the accretion energy
  real(kind=DBL_KIND),parameter :: pi = 3.14159265
  real(kind=DBL_KIND),parameter :: sigma_tom  = 6.65d-25  
 
  real(kind=DBL_KIND) :: r_g, r_in, r_out, T_in4, T_in, p
  real(kind=DBL_KIND) :: L_nu_const, L_nu, L_dnu, S_euv, Seuv_dnu, S_fuv, Sfuv_dnu
  real(kind=DBL_KIND) :: sigma_mean, sigma_H, sigma_dnu, heat_euv_cal, heat_euv_dnu, lumi_euv, lumi_euv_dnu, lumi_fuv, lumi_fuv_dnu
  real(kind=DBL_KIND) :: nu_euv_min, nu_fuv_min, nu_euv_max, nu_fuv_max, dnu, dy, y_in, y_out
  real(kind=DBL_KIND) :: y, nu
  integer :: count_euv, count_fuv, nu_count, y_count, y_max
...
!#endif
  ...
  if (nparticle > 0) then     ! sink particles exist
    ...
    ! call sp_sinkdata2array(np, pmass, pr=pr, pid=pid, pmdot_disk=pmdot_disk, pJ_disk=pJ_disk, pt_prev=pt_prev &
     ...
     !----------EO_added----------!
     allocate(pid(nparticle), pmass(nparticle), pr(MX:MZ, nparticle), pmdot(nparticle), pJ_disk(MX:MZ, nparticle),&
      pt_prev(nparticle),  pdm_disk(nparticle), ptcrt(nparticle))
  ...
  do n = 0, nparticle-1
    ...
    ! rs_info%lum(n_rs)        = lum*1.d-3         ! luminosity [erg s^-1]
...
!     lum = lum * cgs_lsun                                           !luminosity in [erg s^-1]
    ...
    !---define borometric lum---!
    !use accretion rate of BHAD!
    ! lum = eta * mdot * cgs_c**2.d0 /( Unit_v**2.d0)

     !use Eddington luminosity! 
     eta_edd = 1.0d-1
     lum_edd = 4.d0 * pi * cgs_c * cgs_gc * mass * Unit_m  * cgs_mp / sigma_tom  
     lum     = eta_edd * lum_edd    


     !get_spectrum(use DBB model)!
     r_g   = 2.d0 * cgs_gc * mass * Unit_m / cgs_c**2.d0
     r_in  = 3.d0 * r_g
     r_out = 1.d5 * r_g
     T_in4 = 3.d0 * cgs_gc * mass * Unit_m *  mdot * (Unit_m / Unit_t) / (8.d0 * pi * cgs_sigma * r_in **3.d0)
     T_in  = (T_in4)**(1.d0 / 4.d0) 
     p     = 3.d0 / 4.d0 !effective_temp index

     L_nu_const = (4.d0 * pi * cgs_hpk / cgs_c**2.d0) * (r_in** 2.d0 / p) * (cgs_kb * T_in / cgs_hpk) ** (2.d0 / p)
     ...
     heat_euv_cal = 0.d0
     lumi_euv     = 0.d0
     nu_euv_min   = (1.d1 * 1.d3) * cgs_ev / cgs_hpk !10keV
     nu_euv_max   = 13.6 * cgs_ev / cgs_hpk
     count_euv    = 1d3
     dnu          = abs(nu_euv_max - nu_euv_min) / count_euv

     do nu_count = 0, count_euv

        nu    = nu_euv_max + dnu * nu_count
        y_in  = cgs_hpk * nu / (cgs_kb * T_in)
        y_out = y_in * (r_out / r_in)**p 
        y_max = 1d4
        dy    = abs(y_out - y_in) / y_max   
       ...
       do y_count = 0, y_max
          ...
          y = y_in + dy * y_count !non_dimentional variable 
          L_dnu = nu**(3.d0 - 2.d0 / p) *((y ** (2.d0 / p - 1.d0)) / (exp(y) - 1.d0)) * dy
          L_nu  = L_nu + L_dnu 

       end do
    
       L_nu = L_nu_const * L_nu
  
       !for getting S_euv!
       Seuv_dnu = (L_nu / (cgs_hpk * nu)) * dnu
       Seuv_dnu = Seuv_dnu / (1.d0 / Unit_v)
       S_euv    = S_euv + Seuv_dnu

       !for getting alpha_euv! 
       sigma_H    = 6.3 * 1.d-18 * (nu / nu_euv_max)**(-3.d0)
       sigma_dnu  = (L_nu * sigma_H / (cgs_hpk * nu)) * dnu
       sigma_dnu  = sigma_dnu / (Unit_l**2.d0)
       sigma_mean = sigma_mean + sigma_dnu
 
       !for getting heat_euv!
       heat_euv_dnu = (L_nu * (nu - nu_euv_max) / nu) * dnu
       heat_euv_dnu = heat_euv_dnu / Unit_erg
       heat_euv_cal = heat_euv_cal + heat_euv_dnu
 
       !for getting lumeuv!
       lumi_euv_dnu = L_nu * dnu
       lumi_euv_dnu = lumi_euv_dnu / (Unit_erg / Unit_t)
       lumi_euv = lumi_euv + lumi_euv_dnu
     ...
     L_nu         = 0.d0
     lumi_fuv     = 0.d0
     nu_fuv_min   = 13.6 * cgs_ev / cgs_hpk
     nu_fuv_max   = 11.2 * cgs_ev / cgs_hpk
     count_fuv    = 1d3
     dnu          = abs(nu_fuv_max - nu_fuv_min) / count_fuv
     do nu_count = 0, count_fuv

        nu    = nu_fuv_max + dnu * nu_count
        y_in  = cgs_hpk * nu / (cgs_kb * T_in)
        y_out = y_in * (r_out / r_in)**p 
        y_max = 1d4
        dy    = abs(y_out - y_in) / y_max   
       ...
       !for getting S_euv!
       Sfuv_dnu = (L_nu / (cgs_hpk * nu)) * dnu
       Sfuv_dnu = Sfuv_dnu / (1.d0 / Unit_v)
       S_fuv    = S_fuv + Sfuv_dnu

       !for getting lumfuv!
       lumi_fuv_dnu = L_nu * dnu
       lumi_fuv_dnu = lumi_fuv_dnu / (Unit_erg / Unit_t)
       lumi_fuv = lumi_fuv + lumi_fuv_dnu
    ...
    !----------EO_added----------!
     ...
     rda%heat_euv  = heat_euv_cal / S_euv
     rda%hhm       = 0.d0
    #ifdef METAL
     ...
     !----------EO_removed----------!
!     rs_info%Trad(n_rs)       = T_in        ! effective temperature [K]
