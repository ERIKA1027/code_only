...
module radtr
  ...
  integer, dimension(NER:NFZ,ncom), save :: nrads
    ...
    !----------EO_added----------!
    integer, save :: num_cell, num_grid, num_rank
    integer, save, dimension(:), allocatable :: log_gid, log_log_gid, log_rank, log_log_rank

    real(kind=DBL_KIND), parameter :: pi_circ  = 3.14159265 
    real(kind=DBL_KIND), save :: del_V
    real(kind=DBL_KIND), save :: norm_fun
    real(kind=DBL_KIND), save, dimension(:,:), allocatable :: log_cos, log_posR, log_posx, log_posy, log_posz
    real(kind=DBL_KIND), save, dimension(:,:), allocatable :: log_log_cos, log_log_posR, log_log_posx, log_log_posy, log_log_posz
    real(kind=DBL_KIND), save, dimension(:), allocatable :: sum_weight_fun
    !----------EO_added----------!
...
contains
  ...
  subroutine anisotropy_init

      use overBlockCoordinates
      use modelParameter, only : MP_Lmax0

      logical, save :: bool_aniinit = .false.

      integer :: log_gid2, log_rank2, count_rank, count_rank2
      integer :: level, gid, rank
      integer :: num_gid, num_log2, num_gid2
      integer :: i,j,k
      integer,dimension(MX:MZ) :: ijkg
      integer :: grid_iloop, grid_jloop, grid_kloop, loop_rank
      integer, dimension(:), allocatable :: num_log
  
      real(kind=DBL_KIND) :: sign_i, sign_j, sign_k 
      real(kind=DBL_KIND) :: disk_inc, pos_R, cos_disk_cell
      real(kind=DBL_KIND), dimension(MX:MZ) :: pos_injgrid, disk_axis, pos_vec, hmax
      real(kind=DBL_KIND), dimension(:), pointer :: x, y, z


      real(kind=DBL_KIND) :: sum_weight_fun_test, sum_weight_fun_test2
      real(kind=DBL_KIND) :: log_posR2, log_posx2, log_posy2, log_posz2, log_cos2
      ...
      if(.not. bool_aniinit) then
        ...
        num_grid = 8 
        num_cell = (8*8*8)*num_grid
        num_rank = 80

        allocate(log_cos(0:num_cell,0:num_rank))
        allocate(log_posR(0:num_cell,0:num_rank))
        allocate(log_posx(0:num_cell,0:num_rank))
        allocate(log_posy(0:num_cell,0:num_rank))
        allocate(log_posz(0:num_cell,0:num_rank))

        allocate(log_log_cos(0:num_cell,0:num_grid))
        allocate(log_log_posR(0:num_cell,0:num_grid))
        allocate(log_log_posx(0:num_cell,0:num_grid))
        allocate(log_log_posy(0:num_cell,0:num_grid))
        allocate(log_log_posz(0:num_cell,0:num_grid))

        allocate(log_gid(0:num_rank))
        allocate(log_log_gid(0:num_grid))
        allocate(log_rank(0:num_rank))
        allocate(log_log_rank(0:num_grid))
        allocate(num_log(0:num_rank))

        allocate(sum_weight_fun(0:num_rank))
       ...
       ! num_gid_test = 0
        ...
        do grid_iloop = 0, 1
           do grid_jloop = 0, 1
              do grid_kloop = 0, 1
                 ...
                 if(grid_iloop==1) then
                   sign_i = 1.d0
                 end if  
                 if(grid_jloop==1) then
                   sign_j = 1.d0
                 end if  
                 if(grid_kloop==1) then
                   sign_k = 1.d0
                 end if  

                 pos_injgrid(MX) = NI*0.5 * hmax(MX) * sign_i
                 pos_injgrid(MY) = NJ*0.5 * hmax(MY) * sign_j
                 pos_injgrid(MZ) = NK*0.5 * hmax(MZ) * sign_k
 
                 call ob_getIjkgridFromCoordPhys(ijkg, level, pos_injgrid)
                 ...
                 do i = Imin, Imax
                    do j = Jmin, Jmax
                       do k = Kmin, Kmax
                          ...
                          pos_R          = sqrt(x(i)**2.d0 + y(j)**2.d0 + z(k)**2.d0) 
                          pos_vec(MX)    = x(i)/pos_R
                          pos_vec(MY)    = y(j)/pos_R

                          !----------disk equatorial plane----------!
                         ! if (y(j) >= 0.d0) then
                         ! pos_vec(MY)    = (y(j)-0.5*hmax(MY))/pos_R
                         ! end if
                         ! if (y(j) < 0.d0) then
                         ! pos_vec(MY)    = (y(j)+0.5*hmax(MY))/pos_R
                         ! end if
                          ...
                          pos_vec(MZ)    = z(k)/pos_R
                          cos_disk_cell  = dot_product(pos_vec,disk_axis) 

                          !----------disk equatorial plane----------!
                          if ((y(j) < 0.d0 .and. j == Jmax) .or. (y(j) > 0.d0 .and. j==Jmin)) then
                             cos_disk_cell = 0.d0
                          end if
                          !---------disk equatorial plane-----------!
                          ...
                          !---------中心付近は光子注入しない--------!
                          ! if ((abs(x(i))<2.d0*hmax(MX)) .and. (abs(y(j))<2.d0*hmax(MY)) .and. (abs(z(k))<2.d0*hmax(MZ))) then
                          !    cos_disk_cell = 0.d0
                          ! end if
                          !-----------------------------------------!

                          sum_weight_fun(rank) = sum_weight_fun(rank) + 1.d0/(pos_R**2.d0)
                          sum_weight_fun_test = sum_weight_fun_test + 1.d0/(pos_R**2.d0)
 
                          log_posR(num_log(rank),rank) = pos_R
                          log_posx(num_log(rank),rank) = x(i)
                          log_posy(num_log(rank),rank) = y(j)
                          log_posz(num_log(rank),rank) = z(k)
                          log_cos(num_log(rank),rank)  = abs(cos_disk_cell)

                          num_log(rank) = num_log(rank) + 1
                        
                       enddo
                    enddo
                 enddo

               ! num_gid_test = num_gid_test + 1
                ! print*, num_gid_test
        ...
        call mpi_allreduce(sum_weight_fun_test, sum_weight_fun_test2, 1, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, ierr)
        sum_weight_fun_test = sum_weight_fun_test2
        norm_fun            = 1.d0 / sum_weight_fun_test

 
        num_log(:) = 0
        do loop_rank =0, num_rank
           call mpi_allreduce(log_gid(loop_rank), log_gid2, 1, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, ierr)
           log_gid(loop_rank) = log_gid2 
  
            call mpi_allreduce(log_rank(loop_rank), log_rank2, 1, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, ierr)
           log_rank(loop_rank) = log_rank2 
           ...
           do i=Imin, Imax
              do j=Jmin, Jmax
                 do k= Kmin, Kmax
                    ...
                    call mpi_allreduce(log_posR(num_log(loop_rank),loop_rank), log_posR2, 1, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, ierr)
                    call mpi_allreduce(log_posx(num_log(loop_rank),loop_rank), log_posx2, 1, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, ierr)
                    call mpi_allreduce(log_posy(num_log(loop_rank),loop_rank), log_posy2, 1, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, ierr)
                    call mpi_allreduce(log_posz(num_log(loop_rank),loop_rank), log_posz2, 1, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, ierr)
                    call mpi_allreduce(log_cos(num_log(loop_rank),loop_rank), log_cos2, 1, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, ierr)
                        
                    log_posR(num_log(loop_rank),loop_rank) = log_posR2
                    log_posx(num_log(loop_rank),loop_rank) = log_posx2
                    log_posy(num_log(loop_rank),loop_rank) = log_posy2
                    log_posz(num_log(loop_rank),loop_rank) = log_posz2
                    log_cos(num_log(loop_rank),loop_rank)  = log_cos2

                    num_log(loop_rank) = num_log(loop_rank) + 1
       ...
       enddo
        ...
        num_log(:) = 0
        do loop_rank = 0 ,num_rank
           if (log_gid(loop_rank) /= 0) then

             ! call mpi_allreduce(count_rank, count_rank2, 1, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, ierr)      
             ! count_rank = count_rank2
              log_log_gid(count_rank)  = log_gid(loop_rank)
             ! print*, count_rank
 
              log_log_rank(count_rank) = log_rank(loop_rank)
        
              do i=Imin, Imax
                 do j=Jmin, Jmax
                    do k= Kmin, Kmax

                       log_log_posR(num_log(loop_rank),count_rank) = log_posR(num_log(loop_rank),loop_rank)
                       log_log_posx(num_log(loop_rank),count_rank) = log_posx(num_log(loop_rank),loop_rank)
                       log_log_posy(num_log(loop_rank),count_rank) = log_posy(num_log(loop_rank),loop_rank)
                       log_log_posz(num_log(loop_rank),count_rank) = log_posz(num_log(loop_rank),loop_rank)
                       log_log_cos(num_log(loop_rank),count_rank)  = log_cos(num_log(loop_rank),loop_rank)
       ...
       ! call mpi_allreduce(num_gid_test, num_gid2, 1, MPI_DOUBLE_PRECISION, MPI_SUM, MPI_COMM_WORLD, ierr)
 

      end if


  end subroutine anisotropy_init
...
!#endif
  ...
  subroutine injection_step
   ...
   !     del_V = h(MX)*h(MY)*h(MZ)
        ...
        ! point of cell
   !     k = int((pos(MZ)-z(Kmingh))/h(MZ) + 0.5d0)+Kmingh
   !     j = int((pos(MY)-y(Jmingh))/h(MY) + 0.5d0)+Jmingh
   !     i = int((pos(MX)-x(Imingh))/h(MX) + 0.5d0)+Imingh
   ...
   !     i = min(max(i,Imin),Imax)
     ...
     !  do isrc = 0, nsource_glob -1
      ...
      ! num_log  = 0
       ...
       do num_gid = 0, num_grid-1

          gid  = log_log_gid(num_gid)
          rank = log_log_rank(num_gid)
          ...
          do i = Imin, Imax
             do j = Jmin, Jmax
               ...
               ! Nrad_euv(i,j,k) = Nrad_euv(i,j,k) + Ndot_ion*(2.d0*log_log_cos(num_log,num_gid)/norm_fun)*dt/(num_cell*del_V)
               ! Nrad_euv(i,j,k)    = Nrad_euv(i,j,k) + Ndot_ion * norm_fun * (2.d0 * log_log_cos(num_log,num_gid) ) / (log_log_posR(num_log,num_gid)**2.d0)
                Nrad_euv(i,j,k)    = Nrad_euv(i,j,k) + (Ndot_ion * dt * norm_fun / (del_V * log_log_posR(num_log(rank),num_gid)**2.d0))

               ! Nflux_x_euv(i,j,k) = Nflux_x_euv(i,j,k) + Nrad_euv(i,j,k) * cl_til * (log_log_posx(num_log,num_gid)/log_log_posR(num_log,num_gid))
               ! Nflux_y_euv(i,j,k) = Nflux_y_euv(i,j,k) + Nrad_euv(i,j,k) * cl_til * (log_log_posy(num_log,num_gid)/log_log_posR(num_log,num_gid))
               ! Nflux_z_euv(i,j,k) = Nflux_z_euv(i,j,k) + Nrad_euv(i,j,k) * cl_til * (log_log_posz(num_log,num_gid)/log_log_posR(num_log,num_gid))
                Nflux_x_euv(i,j,k) = Nrad_euv(i,j,k) * cl_til * (log_log_posx(num_log(rank),num_gid)/log_log_posR(num_log(rank),num_gid))
                Nflux_y_euv(i,j,k) = Nrad_euv(i,j,k) * cl_til * (log_log_posy(num_log(rank),num_gid)/log_log_posR(num_log(rank),num_gid))
                Nflux_z_euv(i,j,k) = Nrad_euv(i,j,k) * cl_til * (log_log_posz(num_log(rank),num_gid)/log_log_posR(num_log(rank),num_gid))


                #if CHECK_NAN == YES
                if(isNotFinite(Nrad_euv(i,j,k)) .or. isNotFinite(Nflux_x_euv(i,j,k))) then
     ...
     ! enddo   
      ...
      !    gid = log_log_gid(num_gid)
        ...
        ! if (rank /= get_myrank() ) cycle
          ...
          #ifdef M1CLOSER_SEPARATE_FUV_TRANSFER 
          ...
          do i = Imin, Imax
             do j = Jmin, Jmax
                do k = Kmin, Kmax
                 ...
                 ! Nrad_fuv(i,j,k) = Nrad_fuv(i,j,k) + Ndot_ion*(2.d0*log_log_cos(num_log,num_gid)/norm_fun)*dt/(num_cell*del_V)
                 !  Nrad_fuv(i,j,k)    = Nrad_fuv(i,j,k) + Ndot_ion * norm_fun * (2.d0 * log_log_cos(num_log,num_gid) ) / (log_log_posR(num_log,num_gid)**2.d0)
                   Nrad_fuv(i,j,k)    = Nrad_fuv(i,j,k) + (Ndot_ion * norm_fun * dt / (del_V * log_log_posR(num_log(rank),num_gid)**2.d0))

                 !  Nflux_x_fuv(i,j,k) = Nflux_x_fuv(i,j,k) + Nrad_fuv(i,j,k) * cl_til * (log_log_posx(num_log,num_gid)/log_log_posR(num_log,num_gid))
                 !  Nflux_y_fuv(i,j,k) = Nflux_y_fuv(i,j,k) + Nrad_fuv(i,j,k) * cl_til * (log_log_posy(num_log,num_gid)/log_log_posR(num_log,num_gid))
                 !  Nflux_z_fuv(i,j,k) = Nflux_z_fuv(i,j,k) + Nrad_fuv(i,j,k) * cl_til * (log_log_posz(num_log,num_gid)/log_log_posR(num_log,num_gid))

                   Nflux_x_fuv(i,j,k) = Nrad_fuv(i,j,k) * cl_til * (log_log_posx(num_log(rank),num_gid)/log_log_posR(num_log(rank),num_gid))
                   Nflux_y_fuv(i,j,k) = Nrad_fuv(i,j,k) * cl_til * (log_log_posy(num_log(rank),num_gid)/log_log_posR(num_log(rank),num_gid))
                   Nflux_z_fuv(i,j,k) = Nrad_fuv(i,j,k) * cl_til * (log_log_posz(num_log(rank),num_gid)/log_log_posR(num_log(rank),num_gid))

                  #if CHECK_NAN == YES
                   if(isNotFinite(Nrad_fuv(i,j,k)).or. isNotFinite(Nflux_x_fuv(i,j,k))) then
                   ...
                   #ifdef M1CLOSER_SEPARATE_FUV_TRANSFER 
                  ! Nrad_fuv_CO(i,j,k) = Nrad_fuv_CO(i,j,k) + Ndot_ion*(2.d0*log_log_cos(num_log,num_gid)/norm_fun)*dt/(num_cell*del_V)
                  ! Nrad_fuv_CO(i,j,k)    = Nrad_fuv_CO(i,j,k) + Ndot_ion * norm_fun * (2.d0 * log_log_cos(num_log,num_gid) ) / (log_log_posR(num_log,num_gid)**2.d0) 
                   Nrad_fuv_CO(i,j,k)    = Nrad_fuv_CO(i,j,k) + (Ndot_ion * norm_fun * dt / (del_V * log_log_posR(num_log(rank),num_gid)**2.d0)) 

                  ! Nflux_x_fuv_CO(i,j,k) = Nflux_x_fuv_CO(i,j,k) + Nrad_fuv_CO(i,j,k) * cl_til * (log_log_posx(num_log,num_gid)/log_log_posR(num_log,num_gid))
                  ! Nflux_y_fuv_CO(i,j,k) = Nflux_y_fuv_CO(i,j,k) + Nrad_fuv_CO(i,j,k) * cl_til * (log_log_posy(num_log,num_gid)/log_log_posR(num_log,num_gid))
                  ! Nflux_z_fuv_CO(i,j,k) = Nflux_z_fuv_CO(i,j,k) + Nrad_fuv_CO(i,j,k) * cl_til * (log_log_posz(num_log,num_gid)/log_log_posR(num_log,num_gid))
                   Nflux_x_fuv_CO(i,j,k) = Nrad_fuv_CO(i,j,k) * cl_til * (log_log_posx(num_log(rank),num_gid)/log_log_posR(num_log(rank),num_gid))
                   Nflux_y_fuv_CO(i,j,k) = Nrad_fuv_CO(i,j,k) * cl_til * (log_log_posy(num_log(rank),num_gid)/log_log_posR(num_log(rank),num_gid))
                   Nflux_z_fuv_CO(i,j,k) = Nrad_fuv_CO(i,j,k) * cl_til * (log_log_posz(num_log(rank),num_gid)/log_log_posR(num_log(rank),num_gid))


                  ! Nrad_fuv_DUST(i,j,k) = Nrad_fuv_DUST(i,j,k) + Ndot_ion*(2.d0*log_log_cos(num_log,num_gid)/norm_fun)*dt/(num_cell*del_V)
                  ! Nrad_fuv_DUST(i,j,k)    = Nrad_fuv_DUST(i,j,k) + Ndot_ion * norm_fun * (2.d0 * log_log_cos(num_log,num_gid) ) / (log_log_posR(num_log,num_gid)**2.d0) 
                   Nrad_fuv_DUST(i,j,k)    = Nrad_fuv_DUST(i,j,k) + (Ndot_ion * norm_fun * dt / (del_V * log_log_posR(num_log(rank),num_gid)**2.d0)) 


                  ! Nflux_x_fuv_DUST(i,j,k) = Nflux_x_fuv_DUST(i,j,k) + Nrad_fuv_DUST(i,j,k) * cl_til * (log_log_posx(num_log,num_gid)/log_log_posR(num_log,num_gid))
                  ! Nflux_y_fuv_DUST(i,j,k) = Nflux_y_fuv_DUST(i,j,k) + Nrad_fuv_DUST(i,j,k) * cl_til * (log_log_posy(num_log,num_gid)/log_log_posR(num_log,num_gid))
                  ! Nflux_z_fuv_DUST(i,j,k) = Nflux_z_fuv_DUST(i,j,k) + Nrad_fuv_DUST(i,j,k) * cl_til * (log_log_posz(num_log,num_gid)/log_log_posR(num_log,num_gid))

                   Nflux_x_fuv_DUST(i,j,k) = Nrad_fuv_DUST(i,j,k) * cl_til * (log_log_posx(num_log(rank),num_gid)/log_log_posR(num_log(rank),num_gid))
                   Nflux_y_fuv_DUST(i,j,k) = Nrad_fuv_DUST(i,j,k) * cl_til * (log_log_posy(num_log(rank),num_gid)/log_log_posR(num_log(rank),num_gid))
                   Nflux_z_fuv_DUST(i,j,k) = Nrad_fuv_DUST(i,j,k) * cl_til * (log_log_posz(num_log(rank),num_gid)/log_log_posR(num_log(rank),num_gid))
  ...
  ! isrc    = 0
   num_log(:) = 0
        ...
        !----------EO_removed----------!
       ! Nrad_ir(i,j,k) = Nrad_ir(i,j,k) + Ndot_ion/del_V*dt
        !----------EO_removed----------!
          ...
          do i = Imin, Imax
             do j = Jmin, Jmax
                do k = Kmin, Kmax
                   ...
                   !Nrad_ir(i,j,k) = Nrad_ir(i,j,k) + Ndot_ion*(2.d0*log_log_cos(num_log,num_gid)/norm_fun)*dt/(num_cell*del_V)
                  ! Nrad_ir(i,j,k)    = Nrad_ir(i,j,k) + Ndot_ion * norm_fun * (2.d0 * log_log_cos(num_log,num_gid) ) / (log_log_posR(num_log,num_gid)**2.d0)
                   Nrad_ir(i,j,k)    = Nrad_ir(i,j,k) + (Ndot_ion * norm_fun * dt / (del_V * log_log_posR(num_log(rank),num_gid)**2.d0))
 
                  ! Nflux_x_ir(i,j,k) = Nflux_x_ir(i,j,k) + Nrad_ir(i,j,k) * cl_til * (log_log_posx(num_log,num_gid)/log_log_posR(num_log,num_gid))
                  ! Nflux_y_ir(i,j,k) = Nflux_y_ir(i,j,k) + Nrad_ir(i,j,k) * cl_til * (log_log_posy(num_log,num_gid)/log_log_posR(num_log,num_gid))
                  ! Nflux_z_ir(i,j,k) = Nflux_z_ir(i,j,k) + Nrad_ir(i,j,k) * cl_til * (log_log_posz(num_log,num_gid)/log_log_posR(num_log,num_gid))

                   Nflux_x_ir(i,j,k) = Nrad_ir(i,j,k) * cl_til * (log_log_posx(num_log(rank),num_gid)/log_log_posR(num_log(rank),num_gid))
                   Nflux_y_ir(i,j,k) = Nrad_ir(i,j,k) * cl_til * (log_log_posy(num_log(rank),num_gid)/log_log_posR(num_log(rank),num_gid))
                   Nflux_z_ir(i,j,k) = Nrad_ir(i,j,k) * cl_til * (log_log_posz(num_log(rank),num_gid)/log_log_posR(num_log(rank),num_gid))


                   #if CHECK_NAN == YES
                   if(isNotFinite(Nrad_ir(i,j,k)).or. isNotFinite(Nflux_x_ir(i,j,k))) then
