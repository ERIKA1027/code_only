...
! macro for outflow (後でlabel_generatorへ)
#define OUTFLOW_ON
#define OUTFLOW_FW 0.3d0
#define OUTFLOW_FV 0.3d0
...
#undef M_MP    
  ...
  subroutine sp_gravityOfFluid_chi(grav)
    ...
    do
      ...
      call sp_sumUinSphere(ptr%r, sp_SofteningRadius, (/MGX, MGY, MGZ/), grav_ptr, vol)
...
subroutine sp_update_subdisk
  ...
  real(kind=DBL_KIND) :: mass, mdot, radius, lum, Trad, tacc, tkh, dtaccc
#endif
  type(t_spParticle),pointer :: ptr
  ...
  do
     ...
     call ProstFit2(mass, mdot, radius, lum, Trad)
  ...
  subroutine sp_outflow
    ...
    use parameter, only :  Pi
    type(t_spParticle),pointer :: ptr
    integer :: level, kg, jg, ig, gid, rank, gg_ijk, ilog
    integer :: i, j, k, ix, iy, iz
    integer,dimension(MX:MZ) :: ijkgL, ijkgR
    real(kind=DBL_KIND),dimension(MX:MZ) :: pos, posL, posR, h, r_i, jang
    real(kind=DBL_KIND) :: dv, dist_outflw, dist_outflw2, abs_spin, rj_cross, theta, chi, xi, sqrt_theta2, del_theta
    real(kind=DBL_KIND) :: theta_p, theta_m, atan_p, atan_m, sum_eta, sum_local
    real(kind=DBL_KIND) :: wmdotdt, vkep, rstar
    real(kind=DBL_KIND) :: mass, mdot, radius, lum, Trad, dmom, c2_gam_w
    ...
    ilog = 4*MP_spRadius_cell
    allocate(eta(ilog,ilog,ilog))
    ...
    do 
      ...
      pos(MX) = h(MX)*dble(nint(ptr%r(MX)/h(MX)))
      pos(MY) = h(MY)*dble(nint(ptr%r(MY)/h(MY)))
      pos(MZ) = h(MZ)*dble(nint(ptr%r(MZ)/h(MZ)))

      ! evaluate blocks
      posL(:) = pos(:) - 2.d0*sp_SinkRadius 
      posR(:) = pos(:) + 2.d0*sp_SinkRadius 
            ...
            do k = Kmin, Kmax
              do j = Jmin, Jmax
                do i = Imin, Imax
                  ...
                  ! distance from a protostar
                  dist_outflw2 = (x(i)-pos(MX))**2 + (y(j)-pos(MY))**2 + (z(k)-pos(MZ))**2 
                  dist_outflw  = dsqrt(dist_outflw2)

                  if( dist_outflw < 2.d0*sp_SinkRadius .and. dist_outflw >= sp_SinkRadius ) then

                    ix = nint((x(i)-posL(MX))/h(MX)+0.5d0)
                    iy = nint((y(j)-posL(MY))/h(MY)+0.5d0)
                    iz = nint((z(k)-posL(MZ))/h(MZ)+0.5d0)

                    ! vector --------------------------------
                    r_i(MX) = (x(i) - pos(MX))/dist_outflw
                    r_i(MY) = (y(j) - pos(MY))/dist_outflw
                    r_i(MZ) = (z(k) - pos(MZ))/dist_outflw
                    ! ---------------------------------------
                    ...
                    if(abs(sin(0.5*Pi - theta)) .ge. h(MX)/dist_outflw) then
                      ! chi & xi -----------------------------------------
      ...
      ! mass, momentum, energy injection ----------------------------------------------------------------
      do kg = ijkgL(MZ), ijkgR(MZ)
        do jg = ijkgL(MY), ijkgR(MY)
          do ig = ijkgL(MX), ijkgR(MX)
            call get_gid_from_ijkgrid(ig,jg,kg,level,gid,rank)
              ...
              do j = Jmin, Jmax
                do i = Imin, Imax
                  ...
                  if( dist_outflw < 2.d0*sp_SinkRadius .and. dist_outflw >= sp_SinkRadius ) then
                    ...
                    ! mass injection rate
                    ! --------------------
                    w(i,j,k,M_MRHO) = w(i,j,k,M_MRHO) + wmdotdt*eta(ix, iy, iz) 
                    
                    ! --------------------
                    ! momentum injection 
                    ! --------------------
                    dmom = OUTFLOW_FV*vkep*wmdotdt*eta(ix, iy, iz)
                    ...
                    ! energy injection
                    ! --------------------
                    w(i,j,k,M_MP)  = w(i,j,k,M_MP) + wmdotdt*c2_gam_w*eta(ix, iy, iz)
