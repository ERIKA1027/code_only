...
module uniformgrid
  ...
  integer,save :: LevelFinest = Undefi        ! Finest level for making uniform grid
  integer,save :: LevelResolution = Undefi    ! level specifing resolution
...
contains
  ...
  subroutine uniformgrid_write(xmin, ymin, zmin, xmax, ymax, zmax, level, interpolate, prefix)
    ...
    if ( level > LevelMax ) then
       call print_msg('*** uniform_write: level is larger than LevelMax: level = '// trim(num2char(level))//' LevelMax = ' // trim(num2char(LevelMax)))
    ...
    ! call ug_fld(x, y, z, Time(LevelFinest), Step(LevelFinest) )     ! KS MODIFIED
  ...
  subroutine ug_write(x,y,z,time,step)
    ...
    open(FH, file=fn, form='unformatted', access='stream') ! emulating form='unformatted'
    header = lenint*4 + kind(step) + kind(time)
    write(FH) header
    write(FH) size(x4), size(y4), size(z4), Mmax-Mmin+1, step, time  ! size of u
    write(FH) header
...
!!$    print *, 'read in ug_write' 
  ...
  subroutine ug_fld(x,y,z,time,step)
    ...
    integer(kind=LLONG_KIND) :: ivol, iskip
    if( get_myrank() /= PRIMARY_RANK ) return
    ni = size(x,1)
    nj = size(y,1)
    nk = size(z,1)
    ...
    ivol=int(ni, LLONG_KIND)*int(nj, LLONG_KIND)*int(nk, LLONG_KIND)*int(idepthv, LLONG_KIND)
    iskip = 0
  ...
  subroutine ug_interp(uf, uc, rectf, rect, interpolate)
    ...
    call transferU( ut, uf, rectt, rectf ) ! level=6 segmantation falut
    call deallocateU( ut )
...
#undef SZD
  ...
  subroutine ug_DownSize(dlev, rectg)
    ...
    real(kind=DBL_KIND),dimension(:,:),allocatable :: uc, dummy
    logical :: exist
    if (dlev <= 0) return
    myrank = get_myrank()
    ...
    do level = 1, dlev
       ...
       do m = Mmin, Mmax
          ...
          if ( mod( pL%p(MZ), 2 ) /= 0 ) then
             allocate(dummy(ARRAYSIZE2(uf)))
!!$             print *, 'read in ug_DownSize1' 
             read(unit=FH) dummy
             deallocate(dummy)
          end if
          do kob = kcs, kce
!!$             print *, 'read in ug_DownSize2' 
             read(FH) uf
             do job = jcs, jce
                do iob = ics, ice
...
!!$    print *, 'read in conv_u2w_tempFiles' 
