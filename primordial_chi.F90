...
module primordial
  ...
  real(kind=DBL_KIND), parameter :: Tcmb     = DEF_TCMB  !2.73d0*(1.d0 + zred) ! CMB temp.
  ...
  public :: chemreact, HUVAHHH, yHe, zred, Tcmb, c_H2, CoolSolverExplicit, CoolSolverImplicit, adjust_abundance, chemreact_adptv
  public :: adjust_abundance2
...
contains
  ...
  !-----------------------------------------------------------------------  
  subroutine chemreact(xnH,T_K,Tdust,y,yco,rHpi,rH2pd,rHmpd,dt,tchem, xk_in)
    real(kind=DBL_KIND),intent(IN) :: xnH,T_K,rHpi,rH2pd,rHmpd,dt, Tdust
    ...
    integer,parameter :: itrmax   = 20    !10
    ...
    if (present(xk_in)) then
       ...
       call react_coef(xnH,T_K,rHpi,rH2pd,rHmpd, Tdust, xk)
    end if
 ...
 !          end do                    
  ...
  !-----------------------------------------------------------------------  
  subroutine chemreact_adptv(xnH,T_K,Td,y,yco,rHpi,rH2pd,rHmpd,dt,tchem,force_substep,xk_in)
    real(kind=DBL_KIND),intent(IN) :: xnH,T_K,rHpi,rH2pd,rHmpd,dt,Td
    real(kind=DBL_KIND),intent(INOUT) :: y(0:NCHEM-1),yco
    real(kind=DBL_KIND),intent(OUT) :: tchem
    ...
    integer :: i,n
    integer, parameter :: imax = 10 !substepの時間幅を短くする回数のmax
    integer, parameter :: nmax = 20 !substep数のmax
    ...
    do n=0,nmax
       do i=0,imax
          ...
          if (present(xk_in)) then
             call chemreact(xnH,T_K,Td,y_tmp,yco,rHpi,rH2pd,rHmpd,dt_sub,tchem,xk_in=xk_in) ! Tdは一つ前で計算されているのでよい?HFADDED
          else
             !そうでなければ通常のchemreactを実行
             call chemreact(xnH,T_K,Td,y_tmp,yco,rHpi,rH2pd,rHmpd,dt_sub,tchem)
          end if
  ...
  !-----------------------------------------------------------------------  
  subroutine adjust_abundance(y, yco)
  ...
  ! abundance がおかしな値にならないよう調整する
  !-----------------------------------------------------------------------  
  subroutine adjust_abundance2(y)
    real(kind=DBL_KIND),intent(INOUT) :: y(0:NCHEM-1)    
  ...
  !-----------------------------------------------------------------------  
  subroutine react_coef(xnH,T_K,rHpi,rH2pd,rHmpd, Tdust,xk) 
    real(kind=DBL_KIND),intent(IN) :: xnH,T_K,rHpi,rH2pd,rHmpd,Tdust
    ...
    r_a=0.3464101615d-17*sqrt_T*f_a/(1.d0+4.0d-2*dsqrt(T_K+Tdust)+2.0d-3*T_K+ &
      8.0d-6*T_K*T_K)*MP_Metallicity  ! ダスト数密度を考慮する場合はここに加える HFADDED
    xk(21) = r_a
#else
    xk(21) = 0.d0
#endif
...
! A + B -> C + 2D
#define DRDY_AB_C2D(i, A, B, C, D) \
    ddr_A = xk(i)*y(B)*xnH ;\
    ddr_B = xk(i)*y(A)*xnH ;\
    dr_fdy(A,A) = dr_fdy(A,A) - ddr_A ;\
    dr_fdy(B,A) = dr_fdy(B,A) - ddr_A ;\
    dr_fdy(C,A) = dr_fdy(C,A) + ddr_A ;\
    dr_fdy(D,A) = dr_fdy(D,A) + 2.d0*ddr_A ;\
    dr_fdy(A,B) = dr_fdy(A,B) - ddr_B ;\
    dr_fdy(B,B) = dr_fdy(B,B) - ddr_B ;\
    dr_fdy(C,B) = dr_fdy(C,B) + ddr_B ;\
    dr_fdy(D,B) = dr_fdy(D,B) + 2.d0*ddr_B     


!A+B -> 3B
#define DRDY_AB_3B(i,A,B) \
    ddr_A = xk(i)*y(B)*xnH ;\
    ddr_B = xk(i)*y(A)*xnH ;\
    dr_fdy(A,A) = dr_fdy(A,A) - ddr_A ;\
    dr_fdy(B,A) = dr_fdy(B,A) + 2.d0*ddr_A ;\
    dr_fdy(A,B) = dr_fdy(A,B) - ddr_B ;\
    dr_fdy(B,B) = dr_fdy(B,B) + 2.d0*ddr_B

! A + B -> 2C + B
#define DRDY_AB_2CB(i, A, B, C) \
    ddr_A = xk(i)*y(B)*xnH ;\
    ddr_B = xk(i)*y(A)*xnH ;\
    dr_fdy(A,A) = dr_fdy(A,A) - ddr_A ;\
    dr_fdy(C,A) = dr_fdy(C,A) + 2.d0*ddr_A ;\
    dr_fdy(A,B) = dr_fdy(A,B) - ddr_B ;\
    dr_fdy(C,B) = dr_fdy(C,B) + 2.d0*ddr_B
 
! 3A -> A + B
#define DRDY_3A_AB(i, A, B) \
    ddr_A = 3.d0*xk(i)*y(A)*y(A)*xnH_2 ;\
    dr_fdy(A,A) = dr_fdy(A,A) - 2.d0*ddr_A ;\
    dr_fdy(B,A) = dr_fdy(B,A) + ddr_A 

! 2A + B -> 2 B
#define DRDY_2AB_2B(i, A, B) \
    ddr_A = 2.d0*xk(i)*y(A)*y(B)*xnH_2 ;\
    ddr_B =      xk(i)*y(A)*y(A)*xnH_2 ;\
    dr_fdy(A,A) = dr_fdy(A,A) - 2.d0*ddr_A ;\
    dr_fdy(B,A) = dr_fdy(B,A) + ddr_A      ;\
    dr_fdy(A,B) = dr_fdy(A,B) - 2.d0*ddr_B ;\
    dr_fdy(B,B) = dr_fdy(B,B) + ddr_B

! 2A -> A + 2B
#define DRDY_2A_A2B(i, A, B) \
    ddr_A = 2.d0*xk(i)*y(A)*xnH ;\
    dr_fdy(A,A) = dr_fdy(A,A) - ddr_A ;\
    dr_fdy(B,A) = dr_fdy(B,A) + 2.d0*ddr_A    
...
!         -1.973427e1, 1.678905e4, -2.578611e4, 1.482123e1, -4.890915,&
  ...
  subroutine tot_cool(xch,T_K,xLmbd_tot,xk_in)
    ...
    if (present(xk_in)) then
       ...
       !与えられていなければここで計算
       !get xk(:) from xnH, T_K, and radiation
       call react_coef(xch%nH,T_K,xch%rHpi,xch%rH2pd,xch%rHmpd, xch%Td,xk)
    end if
    ...
    call line_cool(xch%nH,T_K,xch%Td,xch%ychem,xch%yco,xch%xlmbdj,xLmbd_line &
#ifdef METAL
        , rOpi &
#endif
      )
...
#if PHOTOELECTRIC_HEATING == YES
    ph_heat   = ph_heat * 1.d0 / (1.d0 + exp((xch%ychem(3) - 0.01)*1.d5))
  ...
  subroutine chem_cool(xnH,T_K,Td,y,xk,heat,xLmbd_chem)
...
#ifdef METAL
    real(kind=DBL_KIND) :: rtH2d, xL_H2d
#endif
    ...
    !    xL_rec = 1d1 ** (-26.02 + 9.187e-1*log_T - 3.733e-1*log_T2&
    !         + 1.174e-1*log_T3 - 1.707e-2*log_T4 + 8.119e-4*log_T5)* y(3) * y(2) * xnH * xnH ! Case A
    xL_rec = 1d1 ** (-25.87 + 4.958e-1*log_T - 1.052e-1*log_T2&
         + 4.264e-2*log_T3 - 9.165e-3*log_T4 + 5.491e-4*log_T5) * y(3) * y(2) * xnH * xnH ! Case B
    ...
    ! ================ total rate per vol. ==============
    xLmbd_chem =  xL_H2 + xL_ci + xL_rec + xL_HM - xG_pi
#ifdef METAL
  ...
  !-----------------------------------------------------------------------  
  subroutine line_cool(xnH,T_K,Td,y,yco,xlmbdj,xLmbd_line &
#ifdef METAL
        , rOpi &
#endif
    )
    real(kind=DBL_KIND),intent(IN) :: xnH,T_K,y(0:NCHEM-1),Td, yco
#ifdef METAL
    ...
    call OII_OII_ratio(ne, y_Hp, T_K, rOpi, y_OII, y_OIII)
    ...
    ! -----------------  Compton cooling --------------------- 
    xLmbd_cpt = 5.65d-36*((1.d0 + zred)**4)*(T_K - Tcmb)*xnH*y_e
    ...
    call carbonII_cooling(nCII, ne, nH, T_K, Tcmb, dvdr, b_cont, cool_rate_CII) 
    ...
    call COcool(nH,Tcmb,y_H2,xNc_CO,tau_cnt,heating_CO)
    ...
    call oxygenI_cooling(nOI, ne, nH, T_K, Tcmb, dvdr, b_cont, cool_rate_OI)
    xLmbd_OI = nOI * cool_rate_OI                          ! [erg/s/cm^3] 


#if Oxygen_COOL_CAL == YES
    call oxygenII_cooling(nOII, ne,   T_K, Tcmb, dvdr, b_cont, cool_rate_OII)
    xLmbd_OII   = nOII* cool_rate_OII                          ! [erg/s/cm^3] 


    call oxygenIII_cooling(nOIII, ne, nH, T_K, Tcmb, dvdr, b_cont, cool_rate_OIII)
...
print '(A,(1P5E15.7))', "xLmbd_H2, xLmbd_Hep, xLmbd_cpt, xLmbd_Lya, xLmbd_ff: ",&
  ...
  function c_H2(T_K)
    ...
    real(kind=DBL_KIND) :: xlT
      ...
      xlT = dlog10(T_K)
      call linear(xlTa,ca,30,xlT,c_H2)
      return
    contains
    !-----------------------------------------------------------------------
    ! liner interpolation
...
!    print '(A,(1P5E15.7))', "(BEFORE) T_K,pr,xmu,gamma,xch%nH: ",T_K,pr,xmu,gamma,xch%nH !KS DEBUG    
    ...
    call react_coef(xch%nH,xch%Tg,xch%rHpi,xch%rH2pd,xch%rHmpd, xch%Td, xk)    

    !update y
    !call chemreact(xnH,T_K,y,rHpi,rH2pd,rHmpd,dt,t_chem)
    call chemreact_adptv(xch%nH,xch%Tg,xch%Td,xch%ychem,xch%yco,xch%rHpi,xch%rH2pd,xch%rHmpd,xch%dt,t_chem,xk_in=xk)

    !update yco
    call update_yco(xch%nH,xch%Tg, xch%ychem, xch%yco, xch%rcopd, xch%dt) 
...
subroutine UpdateTY_with_Tn(xch, T_n, force_substep)
    ...
    call react_coef(xch%nH,T_n,xch%rHpi,xch%rH2pd,xch%rHmpd,xch%Td,xk)    
    ...
    if(xch%nH > 1e11 .or. bool_force_substep) then        !密度高いときやCoolSolver側からsubstepを切ることを要請された場合（温度変化が大きい場合に対応）
       ...
       call chemreact_adptv(xch%nH,T_n,xch%Td,xch%ychem,xch%yco,xch%rHpi,xch%rH2pd,xch%rHmpd &
    ...
    ! update co
    call update_yco(xch%nH,T_n, xch%ychem, xch%yco, xch%rcopd, xch%dt)
