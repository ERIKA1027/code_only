...
contains
  ...
  subroutine art_RayTracing
...
#ifdef METAL
    real(kind=DBL_KIND),dimension(:,:,:),pointer :: kgfuv, kdph ! FUV lumi and dust heating rate
#endif
    ...
    logical :: flg
    integer :: req
    integer,parameter :: maximum_time_debug = 30  ! time when stop run and output debug message in sec
    !---- KS DEBUG --  KS DEBUG --  KS DEBUG --  KS DEBUG ---- !
...
!    use modelParameter, only : MP_SubDiskAbs
    ...
    real(kind=DBL_KIND) :: tempK, nH, dl, dtau_euv, dv, dNcnH
    real(kind=DBL_KIND),dimension(:,:,:),pointer :: rho, p, yhn, yh2, yhp, khpi,heat_hpi,fx_hpi,fy_hpi,fz_hpi !EUV RT    
#if MODEL_ART == 2
    real(kind=DBL_KIND),dimension(:,:,:),pointer :: kh2pd !FUV RT
    real(kind=DBL_KIND) :: dNcH2 ! column density of H2
#endif
#ifdef METAL
    real(kind=DBL_KIND),dimension(:,:,:),pointer :: kgfuv, kdph, Td 
    real(kind=DBL_KIND) :: depsi
#endif
!HFADDED
    ...
    do
       ...
       if (MP_SubStrmAbs == 1) then
          ...
          if (subStrm_flag == 1) then
             tau(0) = tau(0) + 1000.  !電離光子のtauを十分大きくして、シンク内で全部吸収されていることを実現
          end if
       end if
...
#ifdef METAL  ! HFADDED
          call depsilon(Td(i,j,k), depsi)
          call art_GetDNcnH(rho(i,j,k),dl,dNcnH) ! !!cuation!! dNcnHはダストの融解率と柱密度の積
#endif
       ...
       if (sourceCell_flag == 0) then !光源セルでなければ光反応率を格納（光源セルでの光反応は無視, 皮の部分では光反応を考慮, KS TODO）
          ...
          call art_StoreRadEffect(isrc,Hpix,tau,ndir,dl,dv,khpi(i,j,k),heat_hpi(i,j,k), dNcnH, rho(i,j,k), yhn(i,j,k) &
#ifndef NO_RADFORCE
...
#endif !MODEL_ART
#ifdef METAL
               , kgfuv(i,j,k), kdph(i,j,k) &
#endif
               )
       end if

       !update tau(0) <- tau of EUV
       tau(0) = tau(0) + dtau_euv !new tau
       ray%tau(0) = tau(0) !ここでrayのtauも更新しておかないと、art_SplitRayの際にうまく情報が伝わらないので注意
       ...
       tau(1) = tau(1) + dNcH2 !new column density
       ray%tau(1) = tau(1)
#endif

! HFADDED
#ifdef METAL
       tau(BIN_DUST_UV) = tau(BIN_DUST_UV) + dNcnH * depsi  ! ダスト融解まで考慮したeffectiveな柱密度
       ray%tau(BIN_DUST_UV) = tau(BIN_DUST_UV)
#endif
  ...
  contains
    ...
    subroutine check_abs_subDisk(xi,yj,zk,spos,rho,yhn,yh2,subDisk_flag)
      ...
      if (r_in**2 < r2 .and. r2 < r_out**2 ) then
         ...
         if (rho*Unit_rho/(cgs_mh*(1.+4.*yHe)) > n_cr .and. yhn+2.d0*yh2 > y_cr) then
       ...
       print '(A,I4,5(A,1P1E10.2))', "(ART subDisk) ray end: myrank=", &
            ...
            ", nH=", rho*Unit_rho/(cgs_mh*(1.+4.*yHe)),", n_cr=", n_cr,", yhn=", yhn,", yh2=", yh2
#endif
         end if
      end if
    ...
    subroutine check_strm_in_sink(xi,yj,zk,spos,lum,x_euv,rho,subStrm_flag)
      ...
      if (r_in**2 < r2 .and. r2 < r_out**2 ) then
         Ndot_ion = lum * x_euv
         nH = rho*Unit_rho/(cgs_mh*(1.+4.*yHe))
       ...
       print '(A,I4,4(A,1P1E10.2))', "(ART subStrm) EUV absorbed: myrank=", &
            ...
            ", nH=", rho*Unit_rho/(cgs_mh*(1.+4.*yHe)),", r_Strm=", r_Strm/cgs_au
#endif
         end if
      end if
...
!#endif
  ...
  !-------------------------------------------------------------------------
  ! get column density of nH from cell data and dl
  !-------------------------------------------------------------------------
  subroutine art_GetDNcnH(rho,dl,dNcnH)
    real(kind=DBL_KIND),intent(IN) :: rho,dl
    real(kind=DBL_KIND),intent(OUT) :: dNcnH
    real(kind=DBL_KIND) :: nH,dl_cgs
  ...
  !-------------------------------------------------------------------------
  subroutine art_StoreRadEffect(isrc,Hpix,tau,ndir,dl,dv,khpi,heat_hpi,dNcnH, rho, yhn &
#ifndef NO_RADFORCE
    ...
    real(kind=DBL_KIND),intent(IN) :: dl,dv, dNcnH, rho, yhn
    real(kind=DBL_KIND),intent(INOUT) :: khpi,heat_hpi
    real(kind=DBL_KIND) :: lum,dF,dF_EUV,df_hpi,dl_cgs,phi,theta,alpha_euv,heat_euv, tau_fuv, xnH
    ...
    real(kind=DBL_KIND) :: fss,NcH2,lum_FUV,dF_FUV
...
#endif
    real(kind=DBL_KIND) :: tau_uv
#ifdef METAL
    real(kind=DBL_KIND),intent(INOUT) :: kgfuv, kdph
    real(kind=DBL_KIND) :: tau_duv, dtau_dir, tau_dir, dF_tot, ddph,test
    real(kind=DBL_KIND) :: exp_dir, exp_fuv
#endif

    xnH = rho*Unit_rho/(cgs_mh*(1.+4.*yHe)) !in cm^-3 
    ...
    ! HFADDED
    #ifdef METAL
        tau_duv = tau(BIN_DUST_UV) * sig_duv  !optical depth of dust grainn
        tau_fuv = tau(BIN_DUST_UV) * sig_fuv ! dust optical depth for fuv
        tau_dir = tau(BIN_DUST_UV) * sig_dir 
        tau_uv  = tau(0) + tau_duv            !total optical depth for uv
        dtau_dir= dNcnH * sig_dir              !local optical depth of dust grain for stellar irradiation

        test    = dsign(0.5d0, tau_dir - 1.d2)
        exp_dir = dexp(-tau_dir)*(0.5d0-test)

        !if(tau_dir  < 1.d2) then
        !    exp_dir = exp(-tau_dir)
        !else
        !    exp_dir = 0.d0
        !endif
    ...
    #else
        tau_uv  = tau(0)                   !total optical depth for uv
        tau_fuv = 0.d0
    #endif
    ...
    !effect of EUV
    if (tau_uv < tau_max_euv) then !tau > tau_max_euvのときは、すでに十分弱くなっているのでEUVの寄与はゼロとする
         dF_EUV = dF * rs_info%x_euv(isrc) *exp(-tau_uv)    ! delta EUV flux

         !print *, 'tau_uv', tau_duv, tau(0)
         
         !radiation source info
         alpha_euv=rs_info%alpha_euv(isrc)
         heat_euv=rs_info%heat_euv(isrc)

         !store radiation effects on a cell
         khpi = khpi + dF_EUV*alpha_euv                            !ionization rate per H particle [s^-1]
         heat_hpi = heat_hpi + dF_EUV*alpha_euv*heat_euv           !heating rate per H particle [erg s^-1]

#ifndef NO_RADFORCE
         ! HFADDED
#ifdef METAL
         dF_tot = dF * exp_dir  ! energy flux 
         df_hpi = (dF_EUV * (alpha_euv*(heat_euv+chi_H*cgs_ev)) /cgs_c)*yhn + dF_tot * sig_dir / cgs_c ! [ g cm s^{-2}]
#else
         df_hpi = dF_EUV*alpha_euv*(heat_euv+chi_H*cgs_ev)/cgs_c*yhn ! [ g cm s^{-2}]
#endif
         df_hpi = df_hpi / (cgs_mh*(1.+4.*yHe)) / Unit_acc           ! [ cm s^{-2} ] => [no D]         

         fx_hpi = fx_hpi + df_hpi*ndir(MX)    ! [noD]                 !force in x per H particle [g cm s^-2]
         fy_hpi = fy_hpi + df_hpi*ndir(MY)    ! [noD]                 !force in y per H particle [g cm s^-2]
         fz_hpi = fz_hpi + df_hpi*ndir(MZ)    ! [noD]                 !force in z per H particle [g cm s^-2]
#endif    
    end if
...
#ifdef METAL
    if(tau_fuv < tau_max_euv) then

        exp_fuv = exp(-tau_fuv)
        kgfuv   = kgfuv + dF*rs_info%lumfuv(isrc)*exp_fuv/MP_FISRF                               

        NcH2 = tau(1)
        fss =  fss_DB98v2(NcH2)
        !    fss =  1d0 !KS DEBUG
        !lum_FUV = rs_info%x_fuv(isrc)*rs_info%lum(isrc)/N_pix*fss 
...
#ifdef METAL
    if(dtau_dir < 1.d-5)then
        kdph  = kdph + dF_tot * dtau_dir / (dl*Unit_l)  ! [erg s^-1 cm^-3]
    else if(tau_dir <  tau_max_euv) then
        kdph  = kdph  + dF_tot * (1.d0 - exp(-dtau_dir)) / (dl*Unit_l)  ! [erg s^-1 cm^-3]
    else
        kdph  = kdph  + dF_tot / (dl*Unit_l)  ! [erg s^-1 cm^-3]
    endif

    !ダストの加熱率を評価する際に融解の効果は考慮しない
    !if(kdph > 1.d-4) then
    !    print *, kdph, dF, tau_dir, dtau_dir, depsi
    !endif

#endif

! #ifdef ART_DEBUG
!        print '(A, I0, (1P10E15.7))', "(StoreRadEffect) ", N_pix, lum, &
!             rs_info%x_euv(isrc)*rs_info%lum(isrc), ray%tau(0), ray%dist/SinkRadius, dl*Unit_l, (dv)**(1./3)*Unit_l, dF, alpha_euv, khpi
! #if MODEL_ART == 2
!        print '(A, I0, (1P10E15.7))', "KS DEBUG ART: ", N_pix, lum_FUV, &
!             ray%dist*Unit_l/cgs_au, NcH2, lum_FUV, dl*Unit_l, dF_FUV, kh2pd
! #endif ! MODEL_ART
! #endif ! ART_DEBUG
...
#if MODEL_ART == 1
    if(tau(0) > TAU_ENDRAY) then
        #ifdef METAL ! HFADDED
            if(tau(BIN_DUST_UV) > NcnH_max) bool = .TRUE. 
        #else
            bool = .TRUE.
        #endif
#ifdef ART_DEBUG
       print '(A,I4,3(A,1P1E10.2))', "(EndCond) ray end due to tau > TAU_ENDRAY: myrank=", &
            myrank,", dist=", dist,", tau=", tau(0),", TAU_ENDRAY=", TAU_ENDRAY
#endif
    end if
#elif MODEL_ART == 2
    if(tau(0) > TAU_ENDRAY .and. tau(1) > NcH2_max) then
        #ifdef METAL ! HFADDED
            if(tau(BIN_DUST_UV) > NcnH_max) bool = .TRUE. 
        #else
            bool = .TRUE.
       ...
       print '(A,I4,5(A,1P1E10.2))', "(EndCond) ray end due to tau > TAU_ENDRAY: myrank=", &
            myrank,", dist=", dist,", tau_EUV=", tau(0),", TAU_ENDRAY=", TAU_ENDRAY, &
