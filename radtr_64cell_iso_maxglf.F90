...
!#endif
  ...
  subroutine injection_step
    ...
    !-----------EO_added----------!
    integer :: pos_iloop, pos_jloop, pos_kloop, injection_tot
    !-----------EO_added----------!
...
!      do isrc = 0, nsource_glob -1
     ...
     ! h       = CellWidth(:,level)
      ...
      do pos_kloop = 0,3
         ...
         !-----テストとして最深64セルに入れる---!         
         if (pos_iloop==0) then
            pos(MX) = 0.d0 + (1.d0*h(MX)-h(MX)*0.5)
         else if (pos_iloop==1) then         
            pos(MX) = 0.d0 + (2.d0*h(MX)-h(MX)*0.5)
         else if (pos_iloop==2) then
            pos(MX) = 0.d0 - (1.d0*h(MX)-h(MX)*0.5)
         else
            pos(MX) = 0.d0 - (2.d0*h(MX)-h(MX)*0.5)
         end if
         ...
         if (pos_jloop==0) then
            pos(MY) = 0.d0 + (1.d0*h(MY)-h(MY)*0.5)
         else if (pos_jloop==1) then
            pos(MY) = 0.d0 + (2.d0*h(MY)-h(MY)*0.5)
         else if (pos_jloop==2) then
            pos(MY) = 0.d0 - (1.d0*h(MY)-h(MY)*0.5)
         else
            pos(MY) = 0.d0 - (2.d0*h(MY)-h(MY)*0.5)
         end if
         ...
         if (pos_kloop==0) then
            pos(MZ) = 0.d0 + (1.d0*h(MZ)-h(MZ)*0.5)
         else if (pos_kloop==1) then
            pos(MZ) = 0.d0 + (2.d0*h(MZ)-h(MZ)*0.5)
         else if (pos_kloop==2) then
            pos(MZ) = 0.d0 - (1.d0*h(MZ)-h(MZ)*0.5)
         else
            pos(MZ) = 0.d0 - (2.d0*h(MZ)-h(MZ)*0.5)
         end if
    ...
    do CurrentIndex = Gidmin, ListMax !各グリッド
      do nr = 1, ncom
       ...
       ! call get_flux_radtr(ier, ifx, ify, ifz)
        !----------EO_added----------!
        call get_flux_radtr(ier, ifx, ify, ifz, level) 
        !----------EO_added----------!
...
!  subroutine get_flux_radtr(ier, ifx, ify, ifz)
  !----------EO_added----------!
  subroutine get_flux_radtr(ier, ifx, ify, ifz, level)
  !----------EO_added----------!
...
!----------Eo_added----------!
!if(level /= Lmax ) then
if(level /= Lmax .and. level /= Lmax-1) then
!----------Eo_added----------!
#ifdef SOLVER_RADTR_HLL
              ff = 0.d0
