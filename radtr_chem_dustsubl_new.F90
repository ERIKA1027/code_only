...
module radtr_chem
  ...
  use radiationSource
    ...
    use primordial,only: CoolSolverExplicit, CoolSolverImplicit, adjust_abundance &
      , yHe, c_H2, dbg_flg_prm, chem_vals, chem_vals, get_xmu, ch_CellColl_SNe
  #else
    use primordial,only: CoolSolverExplicit, CoolSolverImplicit, adjust_abundance &
  ...
  use kinzoku, only : dtemp_radtr_ntph, find_dop, find_dros, fdust_solar
#else
  use kinzoku, only : dtemp_radtr_ntph, find_dop, find_dros, fdust_solar
#endif
...
contains
  ...
  subroutine ch_radtrchem(dt_code, dt_hyd, rsm)
  ...
  #ifdef GDISKROTATION
    use modelParameter,only : MP_Tmin,MP_Tmax,MP_GDISK_ENDTIME,MP_Tadi,MP_JeansConst
  #else
    ...
    ! Drain & Bertldi
    real(kind=DBL_KIND),parameter :: Efuv_0  = 5.337025d-14    ! erg em^{-3}
    ...
    real(kind=DBL_KIND) :: Nph_fuv_dust

    !#if FUV_COLUMNDENS_OPTION == 2
    !#endif
  ...
  real(kind=DBL_KIND),dimension(:,:,:),pointer :: vx, vy, vz
    ...
    #endif
  #endif
    real(kind=DBL_KIND) :: Trad, rho_dust, xk_ros, tap1, tap2, d_EradIR, d_NradIR
    ...
    real(kind=DBL_KIND) :: Erad_n

  #ifdef OPTICALLY_THICK_OPTION
    real(kind=DBL_KIND) :: Erad_IR, frhpi_IR
    real(kind=DBL_KIND), dimension(:) :: frad_plus(MX:MZ)
  #endif
  ...
  #ifdef MEI 
    real(kind=DBL_KIND),dimension(:,:,:),pointer :: e_int
  #endif

  #ifdef GDISKROTATION
    real(kind=DBL_KIND) :: p_min
    real(kind=DBL_KIND), dimension(MX:MZ) :: hcell
  #endif
    ...
    do level = Lmin, Lmax
        ...
        do i=Imin, Imax
          do j=Jmin, Jmax
            do k=Kmin, Kmax
              ...
              ! --------------------------------
              !  temperature limit before chem
              ! --------------------------------
#ifdef INCLUDE_SNe
              !derived variables
              if (xch%Tg < MP_Tmin*0.999d0) then !表示を抑制するため係数をかける
              
                !output warning
                if (count_output < max_count) then
                   print '(A, 5(1PE12.4))', &
                        "*** before chem_update: T lower  bound imposed *** (T_K, MP_Tmin, x, y,z)", &
                        xch%Tg, MP_Tmin, x(i),y(j),z(k)
      ...
      xch%rcopd = 0.d0
              ...
              xch%heat = 0.d0
              xch%rHpi = 0.d0
  #elif defined(M1CLOSURE_EUV_TRANSFER)
    ...
    #if defined(M1CLOSURE_FUV_TRANSFER) 
              ...
              !----------EO_added-----------!
              if(sqrt(x(i)**2.d0+y(j)**2.d0+z(k)**2.d0)>MP_dustsubl) then
                sig_euv = xch%fd * rsm%sig_EUV  ! 融解 & Metallicity 込み
                sig_fuv = xch%fd * rsm%sig_FUV
              else
                sig_euv = 0.d0
              ...
              #ifdef INCLUDE_SNe
                if(xch%Tg < SNeCoolTemp) then
                  call ch_CellChemCool(xch)
                else
                  call dtemp_radtr_ntph(rho_cgs, xch%nH, xch%Tg, xch%Td, 0.d0, Erad_n, xch%fd, xch%dt)
                  call ch_CellColl_SNe(xch)

                  ! -----------------------
                  sourceCell_flag = .true.   ! 輻射輸送は計算しない
                  ! -----------------------
                endif
              #else
                call ch_CellChemCool(xch)
              ...
              if (xch%Tg < MP_Tmin) then
                 !output warning
                 if (count_output < max_count) then
                    print '(A, 5(1PE12.4))', &
                         "*** after chem_update: T lower bound imposed *** (T_K, MP_Tmin, x, y,z)", &
                         xch%Tg, MP_Tmin, x(i),y(j),z(k)
                    count_output = count_output+1
                    !suppress warning
                    if (count_output == max_count) then
  ...
  #ifdef MEI 
              e_int(i,j,k) = p(i,j,k) ! over write
  #endif !MEI
              ...
              ! --------------------------------------------
              !    adiabatic eos for galactic rotation
              ! --------------------------------------------
  #ifdef GDISKROTATION
              if(Time(level) < MP_GDISK_ENDTIME) then
                p_min     = (cgs_kb*MP_Tadi/Unit_invmu)/(cgs_amu*1.4d0) / Unit_e  ! rho at nH=1cm^-3
                p(i,j,k)  = p_min *(xch%nH)**(5.e0/3.e0)
  #ifdef MEI 
                e_int(i,j,k) = p(i,j,k) ! over write
  #endif !MEI

                !hcell(:)  = CellWidth(:,Lmin)*0.5d0**(MP_LmaxGd-Lmin)
                !p_min     = (hcell(MX)*MP_JeansConst*rho(i,j,k))**2.d0*MP_Gconst/Pi              
                !if(p(i,j,k) < p_min) p(i,j,k) = p_min
              endif
  #endif
              ...
              ! ====================================
                ...
                Trad = (MP_Crd*xch%EradIR_n/cgs_asb)**(0.25d0)    ! temperature of radiation fieild of IR photon
                ...
                ! ----------------------------
                ! set Delta rad
                Nrad_IR(i,j,k) = xch%EradIR_n*Unit_l3/(MP_PHON*MP_hnu_IR)   ![erg cm^-3] => [noD] 
      ...
      #ifdef OPTICALLY_THICK_OPTION
                Erad_IR  =  Nrad_IR(i,j,k)/Unit_l3*MP_PHON*MP_hnu_IR ! [erg cm^-3]
                frhpi_IR = -4.d0/3.d0*xk_ros*MP_Crd*Erad_IR          ! [erg cm^-3]*[cm^2 g^-1] => [ cm s^-2]
               
                frad_plus(MX)  = frhpi_IR*vx(i,j,k)*Unit_cl          ! [ cm s^-2] 
                frad_plus(MY)  = frhpi_IR*vy(i,j,k)*Unit_cl          ! [ cm s^-2] 
                frad_plus(MZ)  = frhpi_IR*vz(i,j,k)*Unit_cl          ! [ cm s^-2] 

                frad_plus(MX:MZ) = frad_plus(MX:MZ)/Unit_acc*dt_code/dt_hyd ! [ cm s^-2] => [noD]

                fx_hpi(i,j,k) = fx_hpi(i,j,k) + Frx_IR(i,j,k)*tap1 + frad_plus(MX) ![noD]
                fy_hpi(i,j,k) = fy_hpi(i,j,k) + Fry_IR(i,j,k)*tap1 + frad_plus(MY) ![noD]
                fz_hpi(i,j,k) = fz_hpi(i,j,k) + Frz_IR(i,j,k)*tap1 + frad_plus(MZ) ![noD]
      #else
  ...
  subroutine radtr_IRdust(dt_code, dt_hyd)
    ...
    real(kind=DBL_KIND) :: yco_l, xnH, xmu, T_K, Td, Erad, Erad_n, dt, rho_cgs, t_dyn
        ...
        do i=Imin, Imax
          do j=Jmin, Jmax
            do k=Kmin, Kmax
              ...
              ! ------------------------
              fd = rho_dust / fdust_solar
              ! ------------------------


#ifdef RADTR_DIRECT
              call dtemp_radtr_ntph(rho_cgs, xnH, T_K, Td, Erad, Erad_n, fd, dt, dph)
#else
              call dtemp_radtr_ntph(rho_cgs, xnH, T_K, Td, Erad, Erad_n, fd, dt)
#endif
              ...
              Trad = (MP_Crd*Erad_n /cgs_asb)**(0.25d0) 
              ! read opacity
              call rdop_radtr(Trad, rho_dust, xk_ros )

              ! update
              Nrad(i,j,k) = Erad_n*Unit_l3/(MP_PHON*MP_hnu_IR)   ![erg cm^-3] => [noD] 
  ...
  function Nph_recom(Tg, xnH, yHII, ye, dt)
    ...
    Nph_recom = (alpha_A - alpha_B)*nHII*ne*dt/MP_PHON*Unit_l**3 ! [cm^-3] => [nD]
