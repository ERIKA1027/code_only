...
!#endif
  ...
  subroutine injection_step
    ...
    !-----------EO_added----------!
    integer :: pos_iloop, pos_jloop, pos_kloop, injection_tot, pos_loopmax, loop_count
    !-----------EO_added----------!
    integer :: nsource_glob
    integer,dimension(MX:MZ) :: ijkg
    real(kind=DBL_KIND),dimension(:),pointer :: x, y, z
    !-----------EO_added----------!
    real(kind=DBL_KIND):: R_polor, norm_r2, norm_const
    !-----------EO_added----------!
...
!      do isrc = 0, nsource_glob -1
     ...
     ! pos_loopmax = 8
      ...
      pos_loopmax = Imax-Imin+1

      loop_count = 0
      do pos_iloop=Imin, Imax
      do pos_jloop=Jmin, Jmax
      do pos_kloop=Kmin, Kmax

         if (pos_iloop<(pos_loopmax/2)) then
            pos(MX) = 0.d0 + ((pos_iloop+1.d0)*h(MX)-h(MX)*0.5)
         else
            pos(MX) = 0.d0 - ((pos_iloop+1.d0-(pos_loopmax/2))*h(MX)-h(MX)*0.5)
         end if
 
         if (pos_jloop<(pos_loopmax/2)) then
            pos(MY) = 0.d0 + ((pos_jloop+1.d0)*h(MY)-h(MY)*0.5)
         else
            pos(MY) = 0.d0 - ((pos_jloop+1.d0-(pos_loopmax/2))*h(MY)-h(MY)*0.5)
         end if
 
         if (pos_kloop<(pos_loopmax/2)) then
            pos(MZ) = 0.d0 + ((pos_kloop+1.d0)*h(MZ)-h(MZ)*0.5)
         else
            pos(MZ) = 0.d0 - ((pos_kloop+1.d0-(pos_loopmax/2))*h(MZ)-h(MZ)*0.5)
         end if
 
 !       if(pos(MX)**2.d0+pos(MY)**2.d0+pos(MZ)**2.d0<(MP_dustsubl**2.d0)*0.2) then
       if(pos(MX)**2.d0+pos(MY)**2.d0+pos(MZ)**2.d0<(h(MX)*9-h(MX)*0.5)) then
     !  if(pos(MX)**2.d0+pos(MY)**2.d0+pos(MZ)**2.d0<(h(MX)*9)) then
          loop_count = loop_count+1
          norm_r2 = norm_r2 + 1.d0/(pos(MX)**2.d0 + pos(MY)**2.d0 + pos(MZ)**2.d0)
       end if
      ...
      norm_const = 1.d0 / norm_r2
      injection_tot=loop_count
...
!        Nrad_euv(i,j,k) = Nrad_euv(i,j,k) + Ndot_ion/del_V*dt


        !----------EO_added----------!
!        Nrad_euv(i,j,k) = Nrad_euv(i,j,k) + (Ndot_ion/del_V*dt)/injection_tot
        Nrad_euv(i,j,k) = Nrad_euv(i,j,k) + (Ndot_ion*norm_const/del_V*dt)/R_polor**2.d0
 ...
 !       Nflux_z_euv(i,j,k) = Nrad_euv(i,j,k) * cl_til * z(k)/R_polor
        ...
        !----------EO_added----------!
       ! if(Nrad_euv(i,j,k) /= 0.d0 .or. Ndot_ion /= 0.d0) then
       !   print*, "**********Nrad_euv Ndot_ion is not zero*********", Nrad_euv(i,j,k)
       ! end if
       ! print*, "******pos(MX),x(Imingh),h(MX)*******",pos(MX),x(Imingh),h(MX)
        !=---------EO_added----------!
...
!        Nrad_fuv(i,j,k) = Nrad_fuv(i,j,k) + Ndot_ion/del_V*dt

        !----------EO_added----------!
!        Nrad_fuv(i,j,k) = Nrad_fuv(i,j,k) + (Ndot_ion/del_V*dt)/injection_tot
        Nrad_fuv(i,j,k) = Nrad_fuv(i,j,k) + (Ndot_ion*norm_const/del_V*dt)/R_polor**2.d0
...
!        Nrad_fuv_CO(i,j,k) = Nrad_fuv_CO(i,j,k) + Ndot_ion/del_V*dt

        Nrad_fuv_DUST => get_Ucomp(MEDUSTF, gid)
!        Nrad_fuv_DUST(i,j,k) = Nrad_fuv_DUST(i,j,k) + Ndot_ion/del_V*dt

        !----------EO_added----------!
!        Nrad_fuv_CO(i,j,k) = Nrad_fuv_CO(i,j,k) + (Ndot_ion/del_V*dt)/injection_tot
!        Nrad_fuv_DUST(i,j,k) = Nrad_fuv_DUST(i,j,k) + (Ndot_ion/del_V*dt)/injection_tot
        Nrad_fuv_CO(i,j,k) = Nrad_fuv_CO(i,j,k) + (Ndot_ion*norm_const/del_V*dt)/R_polor**2.d0
        Nrad_fuv_DUST(i,j,k) = Nrad_fuv_DUST(i,j,k) + (Ndot_ion*norm_const/del_V*dt)/R_polor**2.d0
...
!        Nrad_ir(i,j,k) = Nrad_ir(i,j,k) + Ndot_ion/del_V*dt
        ...
        !-----------EO_added----------!
!         Nrad_ir(i,j,k) = Nrad_ir(i,j,k) + (Ndot_ion/del_V*dt)/injection_tot
         Nrad_ir(i,j,k) = Nrad_ir(i,j,k) + (Ndot_ion*norm_const/del_V*dt)/R_polor**2.d0
