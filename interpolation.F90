...
contains
  ...
  subroutine interp_bicubic_xy(uf, uc, bufx, bufxy, ifs, jfs, kfs, ife, jfe, kfe, if0, jf0, kf0)
    real(kind=DBL_KIND),dimension(:,:,:,:),pointer :: uf, uc, bufx, bufxy
  ...
  ! uf ... output array (pointer)
  ! uc ... input array (pointer). Ghost cells will be accessed.
  ! bufx ..... buffer used when interpolation.
  ! bufxy .... buffer used when interpolation.
  ! ifs, jfs, kfs, ife, jfe, kfe .... region of interpolation in fine index
  ! if0, jf0, kf0 ..... origon of fine index
  ! ----------------------------------------------------------------
  subroutine interp_tricubic(uf, uc, bufx, bufxy, ifs, jfs, kfs, ife, jfe, kfe, if0, jf0, kf0, boolx, booly, boolz)
    ...
    integer :: MINGHCELL = 1    ! minimum number of required ghostcells
    integer :: MAXGHCELL = 2    ! maximum number of required ghostcells
...
#define NEAR(nf, nc)  ( (nc)+2*modulo((nf),2)-1 )
    ...
    bool_x = .TRUE.; bool_y = .TRUE.; bool_z = .TRUE. !default
...
#ifdef VERBOSE
       print *, '*** interp_tricubic: Array size is too small. Instead, interp_trilinear is used.', ice-ics+1, jce-jcs+1, kce-kcs+1
#endif !VERBOSE
