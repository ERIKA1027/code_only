...
module radiationSource
  ...
  public :: radiation_source
   ...
   !----------EO_added----------!
   real(kind=DBL_KIND) :: MP_dustsubl
   public :: MP_dustsubl
   !----------EO_added----------!
...
subroutine radiation_source  
  ...
  use sinkParticle
  !---------EO_added----------!
 ! use boundary, only: rho0_bound, cs0_bound 
  !---------EO_added----------!
#ifndef RADSOURCE_SC
 ...
 ! real :: mdot
  ...
  !----------EO_added----------!
   real(kind=DBL_KIND),parameter :: eta = 1.d-1  
   real(kind=DBL_KIND),parameter :: pi  = 3.14159265  
   real(kind=DBL_KIND),parameter :: sigma_tom  = 6.65d-25  
   real(kind=DBL_KIND) :: p, nu0
   real(kind=DBL_KIND) :: nu_euv_min, nu_euv_max, nu_fuv_min, nu_fuv_max, nu_max, nu_min
   real(kind=DBL_KIND) :: S_euv, S_fuv, sigma_mean, heat_cal1, heat_cal2, lumi_euv, lumi_fuv
   real(kind=DBL_KIND) :: eta_edd, lum_edd, lum0, x_dg, lum_edd_dg
   real(kind=DBL_KIND) :: acrate_bhl, lambda
   real(kind=DBL_KIND) :: mdot_norm,mdot
   real(kind=DBL_KIND) :: time_HL, sim_time
  !----------EO_added----------!
...
!     time_HL = (2.0*cgs_gc*1.d4*cgs_msun / ((20.0*1.d5)**3.0)) / Unit_t
!     sim_time = Time(Lmin)


!     if(time_HL>sim_time) then
!       lum = 1.d-4
!     else
...
!       end if
       !lum = eta * mdot * (Unit_m / Unit_t) * cgs_c**2.d0   !for accretion disks
!     end if
...
!      eta_edd = 1.5d-2                                    !for Eddington rate
!      lum_edd = 4.d0 * pi * cgs_c * cgs_gc * 1.d4 * cgs_msun  * cgs_mp / sigma_tom  
!      lum     = eta_edd * lum_edd

      !-------------EO_constlum------------!
!      x_dg       = 1.0 /(1.0+710.0*MP_Metallicity)
!      lum_edd    = 4.d0 * pi * cgs_c * cgs_gc * MP_Mstar * Unit_m * cgs_mp / sigma_tom  
!      lum_edd_dg = x_dg * lum_edd
!      lum        = MP_Lstar * lum_edd_dg
!      MP_dustsubl = (lum* 2.8d2 / (16.0 * pi * MP_kappa_pl * cgs_sigma *((MP_Tsubl)**(4.d0)) ))**(0.5)
!      MP_dustsubl = MP_dustsubl / Unit_l
      !-------------EO_constlum------------!
    ...
    ! lambda     = 1.d-2
    ! acrate_bhl = 4.d0 * pi * cgs_gc**2.d0 * (1.0d4*cgs_msun)**2.d0 * rho0_bound / ((2.d0*1.d6)**2.d0 + cs0_bound**2.d0)**(3/2)
    ! lum        = eta * lambda * acrate_bhl * cgs_c**2.d0       
     !-----------EO_added-----------!
     ...
     !----------EO_added----------!
    ! p      = 1.d0 / 3.d0      !for standard disk
     p      = - 1.5d0         !for super disks?
     ...
     !for alpha_euv!
     sigma_mean    = 6.3d0 * 1.d-18 * lum0 * nu_euv_min**3.d0 *(nu_euv_max**(p-3.d0) - nu_euv_min**(p - 3.d0)) / (cgs_hpk * (nu0**p) * (p - 3.d0))
     !sigma_mean    = 6.3d0 * 1.d-18 * S_euv
...
!     rs_info%Trad(n_rs)       = Trad        ! effective temperature [K] 
    !----------EO_removed----------!
!     rs_info%Trad(n_rs)       = 1.d4        ! effective temperature [K] 
...
!  #endif
     ...
     !----------EO_added-----------!
     rs_info%x_euv(n_rs)      = S_euv / lum       ! the emissivity of euv photons
     rs_info%x_fuv(n_rs)      = S_fuv / lum       !                   fuv
     rs_info%alpha_euv(n_rs)  = sigma_mean / S_euv   ! mean cross section for EUV photons [cm^-2]
     rs_info%heat_euv(n_rs)   = (heat_cal1 - heat_cal2) / S_euv    ! mean heating rate [erg]
     rs_info%hhm(n_rs)        = 0.d0         ! H- dissociation rate at stellar surface [s^-1]
  #ifdef METAL
     rs_info%lumeuv(n_rs)     = lumi_euv / lum      ! ratio of euv photons
     rs_info%lumfuv(n_rs)     = lumi_fuv / lum      ! ratio of fuv photons
     rs_info%sig_euv(n_rs)    = 2.8d2 * cgs_mh     ! dust cross section for euv photons [cm^-2]
     rs_info%sig_fuv(n_rs)    = 2.8d2 * cgs_mh     ! dust cross section for fuv photons [cm^-2]
