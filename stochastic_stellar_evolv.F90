...
!---------------------------------------------------------------------
!       module for stochastic stellar formation 
!---------------------------------------------------------------------
module stochastic_star
  ...
  integer, save :: lnp_tot    ! total number of sinks recorded in this rank

  type t_sp_mass              
    integer :: sid                    ! id of stars
    integer :: id_ini                 ! id for initial mass
    real(kind=DBL_KIND) :: mass              ! initial mass of star
    real(kind=DBL_KIND) :: ftime             ! formation time
    type (t_sp_mass), pointer :: next => null()
  end type t_sp_mass              

  type t_radsc
    integer :: np                     ! id of sinkpartilce in simulation
    integer :: lnp                    ! id in local cpu
    integer :: nstar                  ! number of star
    real(kind=DBL_KIND) :: mtot              ! total mass of star
  ...
  real(kind=DBL_KIND), save, allocatable, dimension(:) :: dismass, disf
  integer, save :: idis

  ! stellar evolution table
  type stellar_evol
    integer :: nmodel
    real(kind=DBL_KIND), pointer, dimension(:) :: Age        ! Age of star [yr]
    real(kind=DBL_KIND), pointer, dimension(:) :: Rstar      ! radius of star [Rsun]
    real(kind=DBL_KIND), pointer, dimension(:) :: Lumi      ! luminosity of star [Lsun]
    real(kind=DBL_KIND), pointer, dimension(:) :: Teff       ! effective temperature [K]
    real(kind=DBL_KIND), pointer, dimension(:) :: xeuv       ! emissivity of stellar EUV photons
    real(kind=DBL_KIND), pointer, dimension(:) :: xfuv       ! emissivity of stellar EUV photons
    real(kind=DBL_KIND), pointer, dimension(:) :: alpha_euv  ! the cross section for photoionization [cm^2]
    real(kind=DBL_KIND), pointer, dimension(:) :: heat_euv   ! mean energy transferred to gas per one photoionization event [erg]
    real(kind=DBL_KIND), pointer, dimension(:) :: lumeuv     ! the ration of EUV
    real(kind=DBL_KIND), pointer, dimension(:) :: lumfuv     ! the ration of FUV
    real(kind=DBL_KIND), pointer, dimension(:) :: sigd_euv   ! cross section of dust grains for EUV [cm^2]
    real(kind=DBL_KIND), pointer, dimension(:) :: sigd_fuv   ! cross section of dust grains for FUV [cm^2]
    real(kind=DBL_KIND), pointer, dimension(:) :: rOII       ! the rOII ratio of photodisociation rate of OII and HI
  ...
  type st_rs_info
    integer :: nstar
    real(kind=DBL_KIND) :: lum
    real(kind=DBL_KIND) :: Teff
    real(kind=DBL_KIND) :: xeuv
    real(kind=DBL_KIND) :: xfuv
    real(kind=DBL_KIND) :: alpha_euv
    real(kind=DBL_KIND) :: heat_euv
    real(kind=DBL_KIND) :: lumeuv
    real(kind=DBL_KIND) :: lumfuv
    real(kind=DBL_KIND) :: sigd_euv
    real(kind=DBL_KIND) :: sigd_fuv
    real(kind=DBL_KIND) :: rOII
  end type st_rs_info

  ! parameter
  real(kind=DBL_KIND), parameter :: cgs_lsun = 3.904d33        ! Solar Luminosity (erg/s) 
...
contains
  ...
  ! ------------------------------------------------
  !   subroutine for stochastic stellar formation
  !
  !   irank  : my rain of MPI
  !   np_sink: sink number
  !   lnp    : local number of sink particle
  !   smass  : sink mass           [Msun]       
  !   smdot  : mass accretion rate [Msun yr^-1]
  !   stime  : simulation time     [yr]
  ! ------------------------------------------------
  subroutine set_radsc_stchast(irank, np_sink, lnp, smass, smdot, stime, rs_info)

    implicit none
    integer, intent(IN) :: irank, np_sink, lnp
    real(kind=DBL_KIND), intent(IN) :: smass, smdot, stime
    ...
    ! -----------------------
    ! update get stellar mass
    ! -----------------------
    call update_sink_mass(irank, np_sink, lnp, smass, smdot, stime)

    
    ! ------------------------------------
    ! get information of radiation sources
    ! ------------------------------------
  ...
  ! --------------------------------------------
  !  calculate information of radiation source
  ! --------------------------------------------
  subroutine get_rsinfo(irank, np_sink, lnp, smass, smdot, stime, rs_info)
    ...
    real(kind=DBL_KIND) :: lumi_tot, sion_tot, sfuv_tot, alpSi, Sion, heatSi, lum_e, lum_f &
    ...
    do
      ...
      if(isp == lnp) then
        ...
        do n=1, nstar-1   ! リスト最後の星は含まない
          
          call get_invidual_star_inf(sinf, rsinfo_star, stime, id_state) 


          if(id_state .ne. 1) then ! this star is alive
            ! --------------------------
        ...
        rs_info%Teff      = 1.d4      ! 有効温度は使用しないので適当な値としておく
  ...
  ! -----------------------------------
  !  get information of invidual star
  ! -----------------------------------
  subroutine get_invidual_star_inf(sinf, rsinfo_star, stime, id_state) 

    implicit none
    real(kind=DBL_KIND), intent(IN)  :: stime  ! [yr]
    type (t_sp_mass), pointer :: sinf
    type(st_rs_info) :: rsinfo_star
    integer, intent(OUT) :: id_state ! 0:live 1:dead

    integer :: id_ini, i
    real(kind=DBL_KIND) :: lifetime, age_star, age_hi
  ...
  ! -----------------------------------
  !  calculate distrbution function
  ! -----------------------------------
  subroutine cal_sinkmass_with_dist(irank, np_sink, lnp, smass, smdot, stime, sptr)
    ...
    real(kind=DBL_KIND), intent(IN) :: smass, smdot, stime
    type(t_radsc), pointer :: sptr
    real(kind=DBL_KIND) :: newsmass
    integer :: id_ini

    ! --------------------------
    if(sptr%mtot > smass) then ! 合計質量がsink 質量を超えている場合はなにもしない
      return 
    endif
    ! -------------------------
  ...
  ! -----------------------------------
  !     get new star mass randamly
  ! ----------------------------------
  subroutine get_newmass(newsmass, id_ini)
    implicit none

    real(kind=DBL_KIND) :: r, newsmass
    integer :: i, id_ini
  ...
  subroutine radsc_init(irank, stime)
    ...
    use modelParameter, only: MP_SEED_STC
#endif
    implicit none

    integer, intent(IN) :: irank
    real(kind=DBL_KIND), intent(IN) :: stime
    character(len=128) :: filename, file, file2, dir 
    real(kind=DBL_KIND) :: ini_mass, ftime
    ...
    real(kind=DBL_KIND), allocatable, dimension(:) :: mass_line, time_line
    real(kind=DBL_KIND) :: tot_smass, mass, dfunc, newsmass
    integer :: FH = 11
    logical :: exist
    type(t_radsc), pointer :: sptr
    type (t_sp_mass), pointer :: sinf
    ...
    ! ---------------------------------------------------------
#ifdef STOCHASTIC_STELLAR_MODEL
    call sgrnd(irank+1+MP_SEED_STC*NPE)
#else
    call sgrnd(irank+1)
#endif

    ! ---------------------------------------------------------
    !               read stellar evolution table
    ! ---------------------------------------------------------
    ...
    call read_env('DIR', dir)
    write(filename, '("Stellar_Fit/stellar_bin.dat")') ! file
    file=concat(dir,filename)
    open(FH, file=file)
    do 
    ...
    do while ( .true. )
      ...
      !print *, np, lnp, sid, ini_mass, ftime
      ! np: sink num, lnp: local sink num, ini_mass: initial mass, ftime: formation time
      ...
      if (stime >= ftime ) then
        ...
        line_num = line_num + 1
        call get_newmass(newsmass, id_ini) ! random number を修正するために空打ちしておく
        ! -----------------------------------------------------------------------
      endif
    ...
    do while (.true.)
      ...
      if (stime >= ftime ) then
        ...
        np_label(lnp_line(iline)) = np_line(iline) ! log np_sink
        ! --------------------------------------------
      endif
    ...
    do lnp = 1, lnp_tot
      ...
      ! log
      sptr%nstar = nstar     ! number of stars
      sptr%mtot  = tot_smass ! total mass of stars  
  ...
  subroutine add_newstar(sptr, sid, smass, ftime, id_ini)
    ...
    real(kind=DBL_KIND), intent(IN) :: smass, ftime
  ...
  subroutine read_stellar_model
    ...
    real(kind=DBL_KIND) :: mass
    real(kind=DBL_KIND) :: Age, Rstar, Lumi, Teff, xeuv, xfuv, alpha_euv, heat_euv, lumeuv &
...
#else
    filename = trim("Stellar_Fit/")//trim(STELLAR_MODEL_FILENAME)
#endif
    file=concat(dir,filename)
    open(FH, file=file)
    ...
    do i=1, nstarmdl
      ...
      do n=1, nmodel
        ...
        StarTable(i)%Lumi(n)      = Lumi*cgs_lsun            ! convert [Lsun] => [cgs]
        ...
        !print *, StarTable(i)%Age(n)       &
        !,StarTable(i)%Rstar(n)     &
        !,StarTable(i)%Lstar(n)     &
        !,StarTable(i)%Teff(n)      &
        !,StarTable(i)%xeuv(n)      &
        !,StarTable(i)%xfuv(n)      &
        !,StarTable(i)%alpha_euv(n) &
        !,StarTable(i)%heat_euv(n)  &
        !,StarTable(i)%lumeuv(n)    &
        !,StarTable(i)%lumfuv(n)    &
        !,StarTable(i)%sigd_euv(n)  &
        !,StarTable(i)%sigd_fuv(n)  &
        !,StarTable(i)%rOII(n)      
      enddo
    enddo
    close(FH)
