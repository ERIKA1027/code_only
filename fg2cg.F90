...
module fg2cg
  ...
  ! 子グリッドを間引くためのバッファ FLP(lri,lrj,lrk)%Bufc(i,j,k,m)
  type t_buf
...
contains
  ...
  subroutine fg2cg_u( level )
    integer,intent(IN) :: level ! fine level
    if (level < 1) return
...
#define SZ ifs(lri):ife(lri),jfs(lrj):jfe(lrj),kfs(lrk):kfe(lrk),Mmin:Mmax
    ! prepare FLP(ndir)%Bufc
    if ( .not. associated(FLP(Left,Left,Left)%Bufc) ) then
       do lrk = Left, Right
          do lrj = Left, Right
             do lri = Left, Right
                allocate( FLP(lri,lrj,lrk)%Bufc( SZ ) )
             end do
          enddo
       enddo
    endif
#undef SZ
  ...
  subroutine com_restrict
    ...
    use grid
    !
    real(kind=DBL_KIND),dimension(:,:,:,:),pointer :: up
    real(kind=DBL_KIND),dimension(ARRAYSIZE_IJKM),target :: ubuf
    integer :: n, rank, lri, lrj, lrk, i, j, k, ranks, rankd, gids, gidd
    !
    ! ifs(LR surf, direction, parity of this grid)
    !     LR,      MX,MY,MZ,  Ev,Od
    ...
    do rank = 0, NPE -1
       do n = Gidmin, GidListNodeMax(Levelf, rank)
          ...
          if ( myrank == ranks ) then ! 送信 rank
             up => get_Up(gids)
             ubuf = up(ARRAYSIZE_IJKM)
             up => ubuf               ! ポインタのつけかえ
             call restrict_u( up, FLP(lri,lrj,lrk)%Bufc ) ! 送信前に restriction
          endif
          PACK_SEND4( FLP(lri,lrj,lrk)%Bufc, myrank, ranks, rankd)
       enddo
    enddo
    ...
    do rank = 0, NPE -1
       do n = Gidmin, GidListNodeMax(Levelf, rank)
          ...
          if (myrank == rankd) then
             call left_or_right(gids, ranks, lri, lrj, lrk)
             UNPACK_RECV4( FLP(lri,lrj,lrk)%Bufc, myrank, ranks, rankd )
             up => get_Up(gidd)
             up(SZ) = FLP(lri,lrj,lrk)%Bufc
          end if
       enddo
    enddo
  end subroutine com_restrict
  !-------------------------------------------------------------------------
  ! restrict primitive variables (u)  from fine to coarse grid
  !-----------------------------------------------------------------------
  subroutine restrict_u( uf, uc )
    use eos
    real(kind=DBL_KIND),dimension(:,:,:,:),pointer :: uf !(IN)
    real(kind=DBL_KIND),dimension(:,:,:,:),pointer :: uc !(OUT)
    real(kind=DBL_KIND),dimension(:,:,:,:),pointer,save :: wf
    real(kind=DBL_KIND) :: dvf, dvc
    integer :: if,jf,kf,ic,jc,kc,ic0,jc0,kc0,if0,jf0,kf0,m
    logical,save :: bool_initialized = .false.

    if ( .not. bool_initialized ) then
       bool_initialized = .true.
       allocate( wf(ARRAYSIZE4(uf)) )
    endif
    ...
    dvf = 1.d0                  ! volume for fine grid
    dvc = dvf*8                 ! volume for coarse grid
    ...
    do m= lbound(uc,4), ubound(uc,4)
       ...
       enddo
    enddo
    call conv_w2u( uc, dvc ) !uc is primitive variable
